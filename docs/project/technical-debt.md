...
| TD-028 | Execution Model                           | In progress     | US159–US171 add durable Orders/reservations/cancellation/API, mandatory baseline Risk Decisions, a paper-only adapter boundary, deterministic versioned fill configuration, deterministic market/limit matching, a single Execution Engine as sole adapter entry, and append-only immutable Fills. Remaining: accounting core (Ledger postings, positions, portfolio) in Epic E10. | RC-16 (ADR-012)                             |
...
- TD-028 — Execution Model (US165–US171 complete; accounting core/Epic E10 pending)
