...
```text
Binance Real I/O uses Wave 1 security, Vault, and Exchange Adapter factory.
...
| No Wave 1–3 / ownership redesign        | PASS (intent) |
...
| In                                               | Out                                    |
| ------------------------------------------------ | -------------------------------------- |
| Workspace-scoped Binance connect/test/disconnect | Owning customer secret ciphertext      |
...
| SSRF allowlist to Binance vendor endpoints       | Arbitrary operator URL fetch           |
| Verification Standard + regression expectations  | Bybit/OKX/Kraken security (E02–E04)    |
...
| Outcome               | Required                               |
| --------------------- | -------------------------------------- |
| Vendor endpoints only | Binance official API hosts allowlisted |
