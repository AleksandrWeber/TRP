...
**Security:** [`w5-n11-security-review.md`](./w5-n11-security-review.md)
...
Tests that mock platform worker runtime foundation without proving cross-channel foundation coherence do **not** count as Close evidence.

Do not validate per-channel production transport I/O (N01…N04 transport scope), worker runtime execution (orchestration/retry/scheduler/dead-letter), Live Trading, Wave 5 COMPLETE, or Notification Platform Complete from N11 alone. Validate **Notification Platform Worker Runtime Foundation** outcomes only.

---

## 0. What Close means for W5-N11

| Gate                | Meaning                                                                    | Unlocks                           |
| ------------------- | -------------------------------------------------------------------------- | --------------------------------- |
| **W5-N11 Closed**   | Platform worker runtime foundation evidenced; walkthrough PASS             | V3-N11 advanced for package scope |
| **Wave 5 COMPLETE** | Not claimed from N11 alone                                                 | Requires N01…N11 Close + PO       |
| **Not claimed**     | Live Trading / live orders                                                 | Wave 6 + ADR                      |
| **Not claimed**     | Notification Platform Complete                                             | N01…N11 Close + PO                |
...
| Layer                         | Purpose                                                                      |
| ----------------------------- | ---------------------------------------------------------------------------- |
| Unit validation               | Platform worker runtime foundation mapping; workspace binding; honesty rules |
| Integration validation        | Cross-channel foundation consumption; cross-workspace deny                   |
| UI validation                 | Honest Platform Ready / reserved / per-channel / Running labels              |
| Regression validation         | Wave 1–4, W5-N01…N10 boundaries                                              |
| Product walkthrough           | Platform Worker Runtime Foundation Walkthrough executed in product           |
| Architecture validation       | No second engine; worker runtime foundation extension only; PC-06 preserved  |
...
| Area                                | Must prove                                                                      |
| ----------------------------------- | ------------------------------------------------------------------------------- |
| Worker runtime foundation integrity | Platform Ready requires worker runtime foundation evidence                      |
| Per-channel honesty                 | Reserved-inactive not presented as Connected                                    |
| Integration honesty                 | N05 integration truth not overridden by worker runtime layer                    |
| Delivery honesty                    | N06 delivery truth not overridden by worker runtime layer                       |
| Dispatch honesty                    | N07 dispatch truth not overridden by worker runtime layer                       |
| Queue honesty                       | N08 queue truth not overridden by worker runtime layer                          |
| Workers honesty                     | N09 workers truth not overridden by worker runtime layer                        |
| Worker execution honesty            | N10 worker execution truth not overridden by worker runtime layer               |
...
| No capital side effect              | Worker runtime foundation never places live orders                              |
| No worker runtime execution         | Foundation ≠ worker runtime execution/orchestration/retry/scheduler/dead-letter |
| No fake Running                     | Running label requires real worker runtime round-trip                           |
...
| Area                           | Must prove                                                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| N01…N10 foundation consumption | Per-channel, integration, delivery, dispatch, queue, workers, and worker execution anchors consumed; not redesigned |
| Cross-workspace deny           | A cannot use B worker runtime state                                                                                 |
| PC-06 routing consumption      | Routing SoT unchanged; worker runtime foundation consumes only                                                      |
| Restart recovery               | W5-N11-b/c worker runtime anchors hydrate after restart                                                             |
| Operational continuity         | Platform Readiness projection honest                                                                                |
| Vault boundary                 | Worker runtime foundation retrieves; does not store credentials                                                     |
...
| Area                        | Must prove                                                                                      |
| --------------------------- | ----------------------------------------------------------------------------------------------- |
| Platform Ready              | Only after worker runtime foundation evidence                                                   |
| Per-channel labels          | Honest per N01…N04 channel truth                                                                |
| Reserved                    | Unshipped channels show honest “Not offered”                                                    |
| No Live Trading             | UI never implies live capital from worker runtime foundation                                    |
...
| No fake Running             | UI never shows Running without real worker runtime round-trip                                   |
...
| Area                        | Must prove                                                                    |
| --------------------------- | ----------------------------------------------------------------------------- |
| No second engine            | Notification Delivery worker runtime foundation extension only                |
...
| No Master Plan change       | V3-N11 consumed not revised                                                   |
...
| -------- | ---------------------------------------------------------------------------------- |
| W5-N11-a | Cross-channel worker runtime inventory complete; SURVIVE/EPHEMERAL; honesty frozen |
| W5-N11-b | Durable platform worker runtime anchors on notification-delivery owner             |
| W5-N11-c | Restart hydrate of worker runtime anchors                                          |
| W5-N11-d | Operational continuity / Platform Readiness projection                             |
| W5-N11-e | Close Evidence; Final Integration Verification; walkthrough PASS                   |
...
| --------------------- | -------- | ------------------------------- |
| Documents created     | **PASS** | w5-n11 planning package files   |
| Master Plan alignment | **PASS** | V3-N11 · CM-21 mapped (PO auth) |
...
- W5-N11 Closed — **not claimed**
- Platform worker runtime foundation validation PASS at Close — **not claimed**
