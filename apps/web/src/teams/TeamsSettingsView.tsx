...
import { teamsStatusLabel, teamsVerificationLabel } from './teams';
...
export function TeamsSettingsView({
  ...
  connection: TeamsConnectionProductView | null;
  diagnostics: TeamsDiagnosticsView | null;
  history: readonly NotificationDeliveryListItemView[];
  lastTest: TeamsTestProductView | null;
  loading: boolean;
  acting: boolean;
  error: string | null;
  onBind: () => void;
  ...
  if (loading && !connection) {
    return (
      <section data-testid="teams-settings">
        <p className="text-sm text-slate-500">Loading Teams connection…</p>
      </section>
    );
  }

  return (
    <section className="space-y-6" data-testid="teams-settings">
      <div>
        <p className="text-xs uppercase tracking-wide text-slate-500">Notification channels</p>
        <h2 className="mt-1 text-2xl font-semibold">Teams settings</h2>
        <p className="mt-2 text-slate-400">
          Notification channel only. Incoming Webhook URL stays in Connections / Vault. Teams cannot
        ...
        <div className="mt-3 flex flex-wrap gap-3 text-sm">
          ...
          <Link
            to="/notifications/channels/teams/history"
            className="text-sky-400 hover:text-sky-300"
          >
            Teams delivery history
          </Link>
      ...
      {!connection ? (
        <p className="text-sm text-slate-500" data-testid="teams-empty">
          No Teams connection in this workspace.
        </p>
      ) : (
        <>
          <Panel title="Connection status">
            <dl className="grid gap-3 sm:grid-cols-2 text-sm">
              <Fact label="Status" value={teamsStatusLabel(connection.status)} />
              <Fact label="Verification" value={teamsVerificationLabel(connection)} />
              <Fact label="Bound" value={connection.bound ? 'Yes' : 'No'} />
              <Fact label="Transport" value={teamsTransportLabel(connection.transport)} />
          ...
          <Panel title="Channel bind">
            <div className="space-y-3" data-testid="teams-wizard">
              <p className="text-sm text-slate-400">
                Store a Microsoft Teams Incoming Webhook under Connections first. Binding does not
                mean Connected. Send a test after bind. The webhook URL is never shown here.
              </p>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  data-testid="teams-bind"
                  disabled={acting}
                  onClick={onBind}
          ...
          {diagnostics && (
            <Panel title="Diagnostics">
              <dl className="grid gap-3 sm:grid-cols-2 text-sm">
                <Fact label="Transport" value={teamsTransportLabel(diagnostics.teamsTransport)} />
                ...
                <Fact
                  label="Last delivery"
                  value={
                    diagnostics.lastTeamsDelivery
                      ? `${diagnostics.lastTeamsDelivery.outcome} (${diagnostics.lastTeamsDelivery.deliveryId})`
                      : '—'
                  }
                />
                <Fact
                  ...
                  value={
                    diagnostics.lastTeamsDelivery?.adapterReached
                      ? 'Yes'
                      : diagnostics.lastTeamsDelivery
                        ? 'No'
                        : '—'
          ...
          {lastTest && (
            <Panel title="Last test">
              <p className="text-sm text-slate-300" data-testid="teams-last-test">
          ...
          <Panel title="Recent deliveries">
            {history.length === 0 ? (
              <p className="text-sm text-slate-500">No Teams deliveries yet.</p>
            ) : (
              <ul className="space-y-2 text-sm">
                {history.map((item) => (
                  <li
                    ...
                    data-testid="teams-history-item"
                  >
...
function teamsTransportLabel(transport: string): string {
