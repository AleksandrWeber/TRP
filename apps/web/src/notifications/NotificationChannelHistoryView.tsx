...
export function NotificationChannelHistoryView({
  channelId,
  items,
  search,
  outcome,
  ...
  const label = notificationChannelLabel(channelId);
  return (
    <section className="space-y-6" data-testid="channel-history">
      <div>
        <p className="text-xs uppercase tracking-wide text-slate-500">Notification channels</p>
        <h2 className="mt-1 text-2xl font-semibold">{label} delivery history</h2>
        <p className="mt-2 text-slate-400">
          Existing recorded deliveries for this channel, newest first. History is not a second
          delivery owner and does not send or retry. Latency is not available.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="block space-y-1 text-sm">
          ...
          <input
            ...
            data-testid="channel-history-search"
        ...
        <label className="block space-y-1 text-sm">
          ...
          <select
            value={outcome}
            data-testid="channel-filter-outcome"
      ...
      {loading && <p className="text-sm text-slate-500">Loading {label} deliveries…</p>}
      {!loading && items.length === 0 && (
        <p className="text-sm text-slate-500" data-testid="channel-history-empty">
          No recorded {label} deliveries in this workspace.
        </p>
      )}
