...
export function TelegramHistoryView({
  ...
  return (
    <section className="space-y-6" data-testid="telegram-history">
      <div>
        <p className="text-xs uppercase tracking-wide text-slate-500">Telegram</p>
        <h2 className="mt-1 text-2xl font-semibold">Telegram delivery history</h2>
        <p className="mt-2 text-slate-400">
          Existing recorded Telegram deliveries, newest first. History is not a second delivery
          owner and does not send, retry, or call Bot API.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        <Link to="/notifications/channels/telegram" className="text-sky-400 hover:text-sky-300">
          Telegram settings
        </Link>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="block space-y-1 text-sm">
          ...
          <input
            ...
            data-testid="telegram-history-search"
        ...
        <label className="block space-y-1 text-sm">
          ...
          <select
            value={outcome}
            data-testid="telegram-filter-outcome"
        ...
        <label className="block space-y-1 text-sm">
          ...
          <select
            value={type}
            data-testid="telegram-filter-type"
      ...
      {loading && <p className="text-sm text-slate-500">Loading Telegram deliveries…</p>}
      {!loading && items.length === 0 && (
        <p className="text-sm text-slate-500" data-testid="telegram-history-empty">
          No recorded Telegram deliveries in this workspace.
        </p>
      )}
