import { Link } from 'react-router-dom';
import type { MarketProfileVersionListItemView } from '../shared/api';
import { formatUtc } from '../shared/formatUtc';
import { confidenceLabel } from './market-profile';

export function MarketProfileHistoryView({
  items,
  loading,
  error,
}: {
  items: MarketProfileVersionListItemView[];
  loading: boolean;
  error: string | null;
}) {
  return (
    <section className="space-y-6" data-testid="market-profile-history">
      <div>
        <Link to="/market-profile" className="text-sm text-sky-400 hover:text-sky-300">
          Profile home
        </Link>
        <p className="mt-3 text-xs uppercase tracking-wide text-slate-500">Profile</p>
        <h2 className="mt-1 text-2xl font-semibold">Version history</h2>
        <p className="mt-2 text-slate-400">
          Existing published Profile versions for this workspace. History is not a second owner and
          does not calculate profiles.
        </p>
      </div>

      {error ? (
        <div className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
          {error}
        </div>
      ) : null}

      {loading ? <p className="text-sm text-slate-500">Loading version history…</p> : null}
      {!loading && items.length === 0 ? (
        <p className="text-sm text-slate-500">No Profile versions in this workspace.</p>
      ) : null}

      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item.marketProfileId}>
            <Link
              to={`/market-profile/targets/${encodeURIComponent(item.targetId)}/versions/${item.version}`}
              className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 px-3 py-2 text-sm hover:border-white/20"
            >
              <span>
                {item.marketSymbol} <span className="text-slate-500">v{item.version}</span>
                {item.isLatest ? (
                  <span className="ml-2 text-xs text-emerald-300">latest</span>
                ) : null}
              </span>
              <span className="flex gap-2">
                <span className="rounded-full border border-white/10 px-2 py-0.5 text-xs">
                  {confidenceLabel(item.confidenceLevel)}
                </span>
                <span className="text-xs text-slate-500">{formatUtc(item.publishedAt)}</span>
              </span>
            </Link>
          </li>
        ))}
      </ul>
    </section>
  );
}
