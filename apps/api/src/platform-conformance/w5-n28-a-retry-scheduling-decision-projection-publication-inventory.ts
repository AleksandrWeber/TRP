/**
 * w5-n28-a — Notification Retry Scheduling Decision Projection Publication Inventory & Honest Product Baseline.
 *
 * Discovery and classification only.
 * Not Notification Retry Scheduling Decision Projection Publication implementation. Not production transport I/O.
 * Not runtime Decision Projection Publication / runtime Decision Projection / scheduling decision evaluation / runtime scheduling / eligibility /
 * backoff calculation / execution / retry lifecycle / timers / workers / orchestration.
 * Not Retry Engine / Runtime Publication Engine / Runtime Scheduler / runtime decision engine / Worker / Timer /
 * Scheduler Platform / Workflow Engine / Event Bus.
 * Not W5-N28 COMPLETE. Not Notification Platform Complete. Not Wave 5 CLOSED.
 *
 * Classifications: DECISION | CONFIGURATION | EPHEMERAL | RECOVERABLE | NON-RECOVERABLE
 * W5-N01…N27 foundations consumed as reference patterns only — not reopened.
 */

export const W5_N28_A_SLICE_ID = 'W5-N28-a' as const;

export const W5_N28_A_ALLOWED_OWNERS = Object.freeze([
...
export type W5N28AOwner = (typeof W5_N28_A_ALLOWED_OWNERS)[number];

export const W5_N28_A_SUBSTRATE_OWNERS = Object.freeze([
  'notification-delivery',
  'notification-product',
  'connection-management',
  'secret-vault',
...
export const W5_N28_A_ARTIFACT_KINDS = Object.freeze([
  'command',
  'state',
  'projection',
  'runtime',
...
export type W5N28AArtifactKind = (typeof W5_N28_A_ARTIFACT_KINDS)[number];
export const W5_N28_A_REQUIRED_ARTIFACT_KINDS = W5_N28_A_ARTIFACT_KINDS;

export const W5_N28_A_CAPABILITY_CATEGORIES = Object.freeze([
  'implemented-today',
  'infrastructure-only',
  'planned',
  'not-implemented',
...
export type W5N28ACapabilityCategory = (typeof W5_N28_A_CAPABILITY_CATEGORIES)[number];

export const W5_N28_A_DECISION_CLASSIFICATIONS = Object.freeze([
  'DECISION',
  'CONFIGURATION',
  'EPHEMERAL',
  'RECOVERABLE',
...
export type W5N28ADecisionClassification = (typeof W5_N28_A_DECISION_CLASSIFICATIONS)[number];

export const W5_N28_A_HONEST_PRODUCT_STATES = Object.freeze([
  'not-implemented',
  'infrastructure-only',
  'implemented-today',
  'reserved-inactive',
...
export type W5N28AHonestProductState = (typeof W5_N28_A_HONEST_PRODUCT_STATES)[number];

export const W5_N28_A_FUTURE_RESPONSIBILITIES = Object.freeze([
  'W5-N28-b',
  'W5-N28-c',
  'W5-N28-d',
  'W5-N28-e',
  ...
  'out-of-scope-platform-publication-slice-a',
  'out-of-scope-runtime-decision-projection-publication',
...
export type W5N28AFutureResponsibility = (typeof W5_N28_A_FUTURE_RESPONSIBILITIES)[number];

export type W5N28AInventoryRow = Readonly<{
  artifactId: string;
  artifact: string;
  kind: W5N28AArtifactKind;
  owner: W5N28AOwner;
  purpose: string;
  publicationRole: string;
  classification: W5N28ADecisionClassification;
  persistenceRequirement: string;
  recoveryRequirement: string;
  operationalRequirement: string;
  dependencies: readonly string[];
  capabilityCategory: W5N28ACapabilityCategory;
  currentStatus: string;
  honestyRequirement: string;
  futureW5N28Responsibility: W5N28AFutureResponsibility;
  evidencePath: string;
  existsToday: boolean;
  authorizesPublicationFunctional: false;
  authorizesW5N28Complete: false;
  honestProductState: W5N28AHonestProductState;
  operationalVisibility: string;
  customerVisibility: string;
}>;

const DOC = 'docs/project/version-3/wave-5/w5-n28-product-scope.md';
const PKG = 'docs/project/version-3/wave-5/w5-n28-implementation-package.md';
const OVR = 'docs/project/version-3/wave-5/w5-n28-overview.md';
const SEC = 'docs/project/version-3/wave-5/w5-n28-security-review.md';
...
function row(input: RowInput): W5N28AInventoryRow {
  ...
  return Object.freeze({
    ...
    currentStatus:
      input.currentStatus ??
      (isOut
        ? 'Explicit OUT — must not authorize from W5-N28-a'
        : isHonesty
          ? 'Binding honesty frozen'
          : isMissing
            ? 'Missing — planned future slice'
            : 'Inventoried for W5-N28-a discovery'),
    honestyRequirement: input.honestyRequirement,
    futureW5N28Responsibility: input.futureW5N28Responsibility,
...
const own = (
  ...
  purpose: string,
  classification: W5N28ADecisionClassification,
  future: W5N28AFutureResponsibility,
  honesty: string,
  ...
  row({
    ...
    owner,
    purpose,
    publicationRole: extra.publicationRole ?? 'ownership-boundary',
    classification,
    honestyRequirement: honesty,
    futureW5N28Responsibility: future,
    ...extra,
  });
...
const consume = (
  ...
  row({
    ...
    owner,
    purpose,
    publicationRole: 'consumed-foundation',
    classification: 'RECOVERABLE',
    honestyRequirement: 'Consumed not redesigned; does not authorize publication functional',
    futureW5N28Responsibility: future,
...
const honesty = (
  ...
  row({
    ...
    publicationRole: 'honesty-constraint',
...
const out = (
  ...
  honestyRequirement: string,
  future: W5N28AFutureResponsibility,
  classification: W5N28ADecisionClassification = 'NON-RECOVERABLE',
  owner: W5N28AOwner = 'wave-5-documentation',
) =>
  row({
    artifactId,
    artifact,
...
export const W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY: readonly W5N28AInventoryRow[] =
  Object.freeze([
    // Ownership
    own(
    ...
    own(
      ...
      {
        publicationRole: 'consumed-foundation-ownership',
    ...
    own(
      ...
      'RECOVERABLE',
      'w5-n27-reference',
      'Decision Projection ≠ Decision Projection Publication; N27 not reopened',
      {
        publicationRole: 'consumed-foundation-ownership',
    ...
    own(
      ...
      {
        publicationRole: 'honesty-baseline-ownership',
    ...
    row({
      artifactId: 'missing-unified-platform-decision-projection-publication-view',
    ...
    row({
      artifactId: 'missing-publication-persistence',
      artifact:
        'Decision Projection Publication durable persistence — resolved by W5-N28-b (gap row retained for inventory honesty)',
      kind: 'persistence-candidate',
      owner: 'notification-delivery',
      purpose:
        'Record resolved durable decision projection publication persistence gap after W5-N28-b',
      publicationRole: 'missing-gap-resolved',
      classification: 'RECOVERABLE',
      honestyRequirement:
        'Resolved by W5-N28-b — persistence only; not runtime publication; not runtime decision projection; not scheduling/eligibility/backoff/execution',
      ...
      currentStatus:
        'Resolved by W5-N28-b — see persist-candidate-publication-anchor; gap row retained for inventory honesty',
      evidencePath: N28B,
      existsToday: true,
      customerVisibility: 'not customer-visible — gap resolved by W5-N28-b',
    }),
    row({
    ...
    // CONFIGURATION
    row({
      artifactId: 'config-publication-rule-representation',
      ...
      publicationRole: 'configuration-representation',
      classification: 'CONFIGURATION',
      honestyRequirement: 'CONFIGURATION classification ≠ runtime decision projection',
      futureW5N28Responsibility: 'W5-N28-b',
      persistenceRequirement: 'planned W5-N28-b',
      recoveryRequirement: 'planned W5-N28-c',
      operationalRequirement: 'planned W5-N28-d',
    ...
    // DECISION
    row({
      artifactId: 'publication-result-model-representation',
      ...
      publicationRole: 'decision-representation',
      ...
      recoveryRequirement: 'planned W5-N28-c',
      operationalRequirement: 'planned W5-N28-d',
      dependencies: ['config-publication-rule-representation'],
    ...
    row({
      ...
      futureW5N28Responsibility: 'W5-N28-c',
      persistenceRequirement: 'planned W5-N28-b',
      recoveryRequirement: 'planned W5-N28-c',
      dependencies: [
        'consume-w5-n22-retry-backoff-calculation-anchor',
    ...
    row({
      ...
      publicationRole: 'inventory-discovery',
      classification: 'EPHEMERAL',
    ...
    row({
      ...
      publicationRole: 'consumed-runtime',
      classification: 'RECOVERABLE',
    ...
    row({
      ...
      publicationRole: 'absent-runtime',
      classification: 'NON-RECOVERABLE',
      honestyRequirement: 'No Runtime Scheduler',
      futureW5N28Responsibility: 'out-of-scope-runtime-scheduler',
      capabilityCategory: 'not-implemented',
    ...
    row({
      ...
      futureW5N28Responsibility: 'out-of-scope-runtime-decision-engine',
      capabilityCategory: 'not-implemented',
    ...
    row({
      ...
      publicationRole: 'operator-gap',
      classification: 'EPHEMERAL',
      honestyRequirement: 'No customer-visible feature from W5-N28-a',
      futureW5N28Responsibility: 'out-of-scope-platform-publication-slice-a',
      evidencePath: OVR,
      customerVisibility: 'not customer-visible — absent',
    }),
    row({
    ...
    row({
      ...
      artifact:
        'WorkspaceNotificationPlatformRetrySchedulingDecisionProjectionPublicationAnchor — durable publication anchors (W5-N28-b)',
      kind: 'persistence-candidate',
      owner: 'notification-delivery',
      purpose: 'Durable Decision Projection Publication anchor on notification-delivery owner',
      publicationRole: 'durable-anchor',
      classification: 'RECOVERABLE',
      honestyRequirement:
        'Persistence only; not runtime publication; not runtime decision projection; not scheduling/eligibility/backoff/execution',
      futureW5N28Responsibility: 'honesty-baseline',
      persistenceRequirement: 'notification-delivery',
      recoveryRequirement: 'notification-delivery',
      operationalRequirement: 'platform-readiness',
      ...
      honestProductState: 'infrastructure-only',
      currentStatus:
        'Implemented — durable decision projection publication anchors; restart recovery via W5-N28-c; operational continuity via W5-N28-d',
      evidencePath: N28B_PRISMA,
      existsToday: true,
    ...
    row({
      ...
      publicationRole: 'consumed-state',
      classification: 'RECOVERABLE',
      honestyRequirement: 'Reference only; N24 not reopened',
      futureW5N28Responsibility: 'w5-n24-reference',
      persistenceRequirement: 'w5-n24-reference',
      recoveryRequirement: 'w5-n24-reference',
      operationalRequirement: 'w5-n24-reference',
    ...
    row({
      ...
      futureW5N28Responsibility: 'w5-n22-reference',
      persistenceRequirement: 'w5-n22-reference',
      recoveryRequirement: 'w5-n22-reference',
      operationalRequirement: 'w5-n22-reference',
    ...
    row({
      ...
      futureW5N28Responsibility: 'w5-n23-reference',
      persistenceRequirement: 'w5-n23-reference',
      recoveryRequirement: 'w5-n23-reference',
      operationalRequirement: 'w5-n23-reference',
    ...
    honesty(
      'honesty-inventory-only-not-runtime-decision-projection-publication',
    ...
    // Explicit OUT
    out(
      'out-runtime-decision-projection-publication',
    ...
    out(
      ...
      'Runtime Decision Projection — OUT',
      'No Runtime Decision Projection from publication inventory',
      'out-of-scope-runtime-decision-projection',
    ),
    out(
      'out-runtime-decision-evaluation',
      'Runtime Decision Evaluation — OUT',
      'No Runtime Decision Evaluation from publication inventory',
      'out-of-scope-runtime-decision-evaluation',
    ),
    out(
    ...
    out(
      'out-w5-n27-b',
      'W5-N28-b Durable Persistence — OUT of slice a',
      'Slice a does not authorize W5-N28-b',
      'W5-N28-b',
      'EPHEMERAL',
    ),
    out(
      'out-w5-n27-c',
      'W5-N28-c Restart Recovery — OUT of slice a',
      'Slice a does not authorize W5-N28-c',
      'W5-N28-c',
      'EPHEMERAL',
    ),
    out(
      'out-w5-n27-d',
      'W5-N28-d Operational Continuity — OUT of slice a',
      'Slice a does not authorize W5-N28-d',
      'W5-N28-d',
      'EPHEMERAL',
    ),
    out(
      'out-w5-n27-e',
      'W5-N28-e Close Evidence — OUT of slice a',
      'Slice a does not authorize W5-N28-e',
      'W5-N28-e',
      'EPHEMERAL',
    ),
    out(
    ...
    row({
      ...
      publicationRole: 'reference-pattern',
      classification: 'RECOVERABLE',
      honestyRequirement: 'Reference only; N22 not reopened',
      futureW5N28Responsibility: 'w5-n22-reference',
      persistenceRequirement: 'consumed-not-owned',
    ...
    row({
      ...
      publicationRole: 'deferred-dependency',
      classification: 'NON-RECOVERABLE',
      honestyRequirement: 'Production transports deferred',
      futureW5N28Responsibility: 'out-of-scope-production-transport-i/o',
    ...
    row({
      ...
      artifact: 'W5-N28-a inventory slice operational record',
      kind: 'operational',
      owner: 'wave-5-documentation',
      purpose: 'Mark slice a inventory baseline as operational documentation state',
      publicationRole: 'inventory-operational',
      classification: 'EPHEMERAL',
...
export const W5_N28_A_BINDING_FINDINGS = Object.freeze({
  publicationFunctionalAuthorized: false,
  publicationFunctionsAfterSliceA: false,
...
export const W5_N28_A_EXPLICIT_OUT = Object.freeze([
...
export const W5_N28_A_ARCHITECTURE_CLAIMS = Object.freeze({
  ...
  runtimeDecisionProjectionPublicationIntroduced: false,
  runtimePublicationEngineIntroduced: false,
...
export const W5_N28_A_HONEST_PRODUCT_BASELINE = Object.freeze({
  implementedCapabilities: Object.freeze([
    'None — W5-N28-a inventory only; no customer-visible publication feature',
  ] as const),
  ...
  notYetImplementedCapabilities: Object.freeze([
    ...
    'Operator Decision Projection Publication UI',
...
export function artifactIds(): readonly string[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.map(
    (entry) => entry.artifactId,
  );
}

export function rowsByKind(kind: W5N28AArtifactKind): readonly W5N28AInventoryRow[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.filter(
...
export function rowsByClassification(
  classification: W5N28ADecisionClassification,
): readonly W5N28AInventoryRow[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.filter(
    (entry) => entry.classification === classification,
  );
}

export function rowsRecoverable(): readonly W5N28AInventoryRow[] {
  return rowsByClassification('RECOVERABLE');
}

export function rowsEphemeral(): readonly W5N28AInventoryRow[] {
  return rowsByClassification('EPHEMERAL');
}

export function rowsDecision(): readonly W5N28AInventoryRow[] {
  return rowsByClassification('DECISION');
}

export function rowsConfiguration(): readonly W5N28AInventoryRow[] {
  return rowsByClassification('CONFIGURATION');
}

export function rowsNonRecoverable(): readonly W5N28AInventoryRow[] {
  return rowsByClassification('NON-RECOVERABLE');
}

export function rowsHonestyBoundaries(): readonly W5N28AInventoryRow[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.filter(
    (entry) => entry.kind === 'honesty-boundary',
  );
}

export function rowsExplicitOut(): readonly W5N28AInventoryRow[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.filter(
...
export function rowsByCapabilityCategory(
  category: W5N28ACapabilityCategory,
): readonly W5N28AInventoryRow[] {
  return W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.filter(
