/**
 * W5-N05-a — Notification Platform Integration Conformance Registry.
 *
 * Validates the canonical platform integration inventory and Honest Product baseline.
...
export const W5_N05_A_CONFORMANCE_SLICE_ID = W5_N05_A_SLICE_ID;

export const W5_N05_A_REQUIRED_REPORTS = Object.freeze([
  'w5-n05-a-notification-platform-integration-inventory.md',
...
export function verifyInventoryCompleteness(): Readonly<{
  ok: boolean;
  rowCount: number;
  noRowAuthorizesPlatformIntegrationFunctional: boolean;
  noRowAuthorizesW5N05Complete: boolean;
  ...
  const noRowAuthorizesPlatformIntegrationFunctional =
    W5_N05_A_NOTIFICATION_PLATFORM_INTEGRATION_INVENTORY.every(
      (row) => !row.authorizesPlatformIntegrationFunctional,
    );
  const noRowAuthorizesW5N05Complete = W5_N05_A_NOTIFICATION_PLATFORM_INTEGRATION_INVENTORY.every(
    (row) => !row.authorizesW5N05Complete,
  );
  return Object.freeze({
    ok:
      W5_N05_A_NOTIFICATION_PLATFORM_INTEGRATION_INVENTORY.length >= 45 &&
      noRowAuthorizesPlatformIntegrationFunctional &&
      noRowAuthorizesW5N05Complete &&
      requiredOwnershipRowsPresent,
    rowCount: W5_N05_A_NOTIFICATION_PLATFORM_INTEGRATION_INVENTORY.length,
    noRowAuthorizesPlatformIntegrationFunctional,
    noRowAuthorizesW5N05Complete,
    requiredOwnershipRowsPresent,
  });
...
export function verifyHonestProductBaseline(): Readonly<{
  ...
  platformIntegrationNotAuthorized: boolean;
  deliveryOnlyNotControlPlane: boolean;
}> {
  const implemented = W5_N05_A_HONEST_PRODUCT_BASELINE.implementedCapabilities;
  const noCustomerVisibleImplemented =
    implemented.length === 1 && implemented[0]?.includes('None') === true;
  const infrastructureDocumented =
    W5_N05_A_HONEST_PRODUCT_BASELINE.infrastructureCapabilities.length >= 8;
  const plannedExplicit = W5_N05_A_HONEST_PRODUCT_BASELINE.plannedCapabilities.length >= 1;
  const notImplementedExplicit =
    W5_N05_A_HONEST_PRODUCT_BASELINE.notYetImplementedCapabilities.length >= 5;
  const platformIntegrationNotAuthorized =
    !W5_N05_A_BINDING_FINDINGS.platformIntegrationFunctionalAuthorized;
  const deliveryOnlyNotControlPlane = !W5_N05_A_ARCHITECTURE_CLAIMS.notificationControlPlane;
  return Object.freeze({
    ok:
      ...
      platformIntegrationNotAuthorized &&
...
export function verifyArchitectureIntegrity(): Readonly<{
  ...
  const ownershipUnchanged = !W5_N05_A_ARCHITECTURE_CLAIMS.ownershipBoundariesChanged;
  const noDuplicateSubsystem =
    !W5_N05_A_ARCHITECTURE_CLAIMS.duplicateNotificationSubsystem &&
    !W5_N05_A_ARCHITECTURE_CLAIMS.duplicateRoutingEngine;
  const noMasterPlanChange = !W5_N05_A_ARCHITECTURE_CLAIMS.masterPlanModified;
  const exchangeAdapterUntouched = W5_N05_A_ARCHITECTURE_CLAIMS.exchangeAdapterUntouched;
  const notificationControlPlaneForbidden = !W5_N05_A_ARCHITECTURE_CLAIMS.notificationControlPlane;
  return Object.freeze({
    ok:
      ownershipUnchanged &&
...
export function verifyOwnershipBoundaries(): Readonly<{
  ...
  return Object.freeze({
    ok:
      W5_N05_A_BINDING_FINDINGS.ownershipBoundariesVerified &&
      !W5_N05_A_BINDING_FINDINGS.ownershipBoundariesChanged &&
      !W5_N05_A_ARCHITECTURE_CLAIMS.newPersistenceOwner &&
      substrateOwnersFrozen,
    substrateOwnersFrozen,
    ownershipVerified: W5_N05_A_BINDING_FINDINGS.ownershipBoundariesVerified,
    newPersistenceOwner: W5_N05_A_ARCHITECTURE_CLAIMS.newPersistenceOwner,
  });
}
...
export function verifyHonestyBoundaries(): Readonly<{
  ...
  platformIntegratedNotLiveTrading: boolean;
  ...
  const platformIntegratedNotLiveTrading = ids.has('honesty-platform-integrated-not-live-trading');
  const platformReadyRequiresEvidence = ids.has('honesty-platform-ready-requires-evidence');
  const perChannelNotPlatformComplete = ids.has('honesty-per-channel-not-platform-complete');
  return Object.freeze({
    ok:
      boundaries.length >= 4 &&
      platformIntegratedNotLiveTrading &&
...
export function buildNotificationPlatformIntegrationDiagnostics(): Readonly<{
  ...
  technicalDebtDelta: typeof W5_N05_A_TECHNICAL_DEBT_DELTA;
  ok: boolean;
}> {
  ...
  return Object.freeze({
    ...
    ownership,
    honesty,
    technicalDebtDelta: W5_N05_A_TECHNICAL_DEBT_DELTA,
    ok:
      inventory.ok &&
      honestProduct.ok &&
      architecture.ok &&
      ownership.ok &&
      honesty.ok &&
      !W5_N05_A_BINDING_FINDINGS.platformIntegrationFunctionsAfterSliceA,
  });
}
