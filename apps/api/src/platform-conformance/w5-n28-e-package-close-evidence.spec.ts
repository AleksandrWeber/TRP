...
import { W5_N28_A_ARCHITECTURE_CLAIMS } from './w5-n28-a-retry-scheduling-decision-projection-publication-inventory';
...
import {
  ...
  verifyPublicationFoundationChain,
  verifyGovernanceIntegrity,
  verifyHonestProduct,
  verifyImplementationChain,
  verifyOperationalChain,
  W5_N28_E_APPROVED_SLICES,
  W5_N28_E_ARCHITECTURE_CLAIMS,
  W5_N28_E_BINDING_FINDINGS,
  W5_N28_E_CAPABILITY_EVOLUTION,
  W5_N28_E_DEPENDENCY_CHAIN,
  W5_N28_E_NOTIFICATION_OWNER,
  W5_N28_E_OPERATIONAL_MATURITY,
  W5_N28_E_REQUIRED_REPORTS,
  W5_N28_E_REQUIRED_SLICE_REPORTS,
  W5_N28_E_SLICE_ID,
  W5_N28_E_TECHNICAL_DEBT_DELTA,
  W5_N28_E_TRANSITION_MATRIX,
} from './w5-n28-e-package-close-evidence';
...
describe('W5-N28-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N28_E_SLICE_ID).toBe('W5-N28-e');
    expect(W5_N28_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N28_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N28-a',
      'W5-N28-b',
      'W5-N28-c',
      'W5-N28-d',
    ...
    expect(
      W5_N28_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('implementation chain complete', () => {
    ...
    expect(W5_N28_E_BINDING_FINDINGS.implementationChainComplete).toBe(true);
  });
  ...
  it('dependency chain intact — prior packages closed and consumed, not reopened', () => {
    ...
    expect(W5_N28_E_DEPENDENCY_CHAIN.map((link) => link.packageId)).toEqual([
      'W5-N01',
      'W5-N02',
      'W5-N03',
      'W5-N04',
    ...
    expect(W5_N28_E_BINDING_FINDINGS.dependencyChainIntact).toBe(true);
  });

  it('projection foundation chain integrity holds', () => {
    const foundation = verifyPublicationFoundationChain();
    ...
    expect(W5_N28_E_BINDING_FINDINGS.publicationFoundationChainIntact).toBe(true);
  });
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N28_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });

  it('Honest Product enforcement intact', () => {
    const honest = verifyHonestProduct();
    expect(honest.ok).toBe(true);
    expect(honest.operationalContinuityNotRuntimePublication).toBe(true);
    expect(honest.restartRecoveryNotProductionReady).toBe(true);
    expect(honest.inventoryHonestBaselineIntact).toBe(true);
    expect(honest.publicationFunctionalNotAuthorized).toBe(true);
    expect(honest.publicationDoesNotCalculateEligibilityScheduleOrExecute).toBe(true);
    expect(honest.runtimePublicationAndDecisionProjectionNotClaimed).toBe(true);
    expect(W5_N28_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N28_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N28_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, projection functional, or Wave 5 COMPLETE', () => {
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.w5N28CompleteClaimed).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.publicationFunctionalClaimed).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeDecisionProjectionImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeSchedulingImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.backoffCalculationImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.eligibilityDeterminationImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.executionImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.retryEngineIntroduced).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeDecisionEngineIntroduced).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeSchedulerIntroduced).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N28_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
    expect(diagnostics.publicationFoundation.ok).toBe(true);
  });
});

describe('W5-N28-e package close evidence — integration / planning', () => {
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newRecoveryLogic).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.newOperationalContinuityLogic).toBe(false);
  });

  it('required Close Evidence and slice reports exist', () => {
    for (const name of W5_N28_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N28_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n28-a-inventory.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n28-overview.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n28-validation-plan.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n28-planning-summary.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'wave-5-progress.md'))).toBe(true);
  });

  it('status docs: W5-N28 CLOSED by Product Owner; Wave 5 COMPLETE not claimed', () => {
    const progress = readWave5('wave-5-progress.md');
    const overview = readWave5('w5-n28-overview.md');
    const close = readWave5('w5-n28-close-package-report.md');
    const summary = readWave5('w5-n28-package-summary.md');
    const walkthrough = readWave5('w5-n28-operational-walkthrough.md');
    const finalIntegration = readWave5('w5-n28-final-integration-verification.md');
    const poClose = readWave5('w5-n28-product-owner-close-record.md');
    expect(existsSync(join(WAVE5, 'w5-n28-final-integration-verification.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n28-product-owner-close-record.md'))).toBe(true);
    expect(progress).toMatch(/W5-N28-a|W5-N28-b|W5-N28-c|W5-N28-d|W5-N28-e/);
    expect(progress).toMatch(/CLOSED by Product Owner|W5-N28\s+\*\*CLOSED\*\*/i);
    ...
    expect(overview).toMatch(/CLOSED by Product Owner|W5-N28[\s\S]*CLOSED/i);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /runtime Decision Projection Publication|Runtime Publication|Decision Projection Publication Foundation only|Do NOT declare runtime/i,
    );
  ...
  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N28_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N28_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N28_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N28_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N28_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N28_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(
      /without runtime decision projection publication/i,
    );
    expect(W5_N28_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N28_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W5_N28_E_TECHNICAL_DEBT_DELTA.deferred).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N28_E_TECHNICAL_DEBT_DELTA.deferred).toContain('Product Owner Final Close');
  });
  ...
  it('platform readiness exposes projection continuity without projection-engine controls', () => {
    ...
    expect(view).toMatch(/notificationPlatformRetrySchedulingDecisionProjectionPublication/);
    expect(view).toMatch(/Notification Platform Retry Scheduling Decision Projection Publication/);
    expect(view).not.toMatch(
      /Runtime Decision Engine UI|Scheduler Engine UI|Retry Engine UI|Publication Engine UI/i,
    );
    expect(api).toMatch(/notificationPlatformRetrySchedulingDecisionProjectionPublication/);
    expect(api).toMatch(
      /NotificationPlatformRetrySchedulingDecisionProjectionPublicationContinuityView/,
    );
    expect(readiness).toMatch(/notificationPlatformRetrySchedulingDecisionProjectionPublication/);
  ...
  it('mandatory questions expected answers hold', () => {
    ...
    expect(W5_N28_E_APPROVED_SLICES).toHaveLength(4);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.publicationFunctionalClaimed).toBe(false);
    expect(W5_N28_D_ARCHITECTURE_CLAIMS.operationalContinuityDerived).toBe(true);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeDecisionProjectionImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimePublicationImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeDecisionEvaluationImplemented).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.runtimeSchedulerIntroduced).toBe(false);
    expect(W5_N28_E_ARCHITECTURE_CLAIMS.executionImplemented).toBe(false);
    expect(W5_N28_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N28_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
