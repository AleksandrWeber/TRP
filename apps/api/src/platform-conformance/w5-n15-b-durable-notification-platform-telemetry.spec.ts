...
import {
  ...
  W5_N15_B_CANONICAL_ANCHOR_FIELDS,
  W5_N15_B_DURABLE_COVERAGE,
  W5_N15_B_EXPLICIT_OUT,
  W5_N15_B_NEW_PERSISTED_ARTIFACT_IDS,
  W5_N15_B_NOTIFICATION_OWNER,
  W5_N15_B_PREEXISTING_SURVIVE_ARTIFACT_IDS,
  W5_N15_B_SLICE_ID,
  W5_N15_B_TECHNICAL_DEBT_DELTA,
  W5_N15_B_TRANSITION_MATRIX,
  newPersistedInventoryRows,
  persistedArtifactIds,
  preexistingSurviveInventoryRows,
  verifyInventorySynchronization,
} from './w5-n15-b-durable-notification-platform-telemetry';
  ...
  return {
    workspaceNotificationPlatformTelemetryAnchor: {
      upsert: async ({
        ...
        const key = `${workspaceId}:${telemetryAnchorId}`;
      ...
      }) => rows.get(`${workspaceId}:${telemetryAnchorId}`) ?? null,
    },
    _rows: rows,
  };
}

describe('W5-N15-b durable notification platform telemetry — unit', () => {
  it('persistence correctness: anchor upserts workspace telemetry row', async () => {
    ...
    const loaded = await service.loadAnchor('ws-a', 'telemetry-1');
    expect(loaded).toMatchObject({
      workspaceId: 'ws-a',
      telemetryAnchorId: 'telemetry-1',
      telemetryState: 'anchor-recorded',
    });
    expect(loaded).not.toHaveProperty('telemetryRuntimeState');
    expect(loaded).not.toHaveProperty('telemetryReplayState');
  });

  it('artifact coverage: only approved new SURVIVE row is persisted by this slice', () => {
    expect([...persistedArtifactIds()].sort()).toEqual(
      [...W5_N15_B_NEW_PERSISTED_ARTIFACT_IDS].sort(),
    );
    const rows = newPersistedInventoryRows();
    expect(rows.length).toBe(W5_N15_B_NEW_PERSISTED_ARTIFACT_IDS.length);
    for (const row of rows) {
      expect(row.artifactId).toBe('persist-notification-platform-telemetry-anchor');
  ...
  it('pre-existing SURVIVE rows remain on notification-delivery owner', () => {
    ...
    expect(preexisting.length).toBe(W5_N15_B_PREEXISTING_SURVIVE_ARTIFACT_IDS.length);
    expect(
      preexisting.every(
        (row) =>
  ...
  it('ownership: platform telemetry persistence remains on notification-delivery owner only', () => {
    expect(W5_N15_B_NOTIFICATION_OWNER).toBe('notification-delivery');
    for (const row of W5_N15_B_DURABLE_COVERAGE) {
      expect(row.owner).toBe('notification-delivery');
    }
  });

  it('canonical anchor fields are frozen for W5-N15-b', () => {
    expect(W5_N15_B_CANONICAL_ANCHOR_FIELDS).toEqual(
...
describe('W5-N15-b durable notification platform telemetry — integration', () => {
  it('persistence lifecycle: no recovery / telemetry runtime / functional claims from this slice', () => {
    expect(W5_N15_B_SLICE_ID).toBe('W5-N15-b');
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.automaticRestartRecovery).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.operationalContinuityGuaranteed).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.platformTelemetryImplementation).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.productionTransportIo).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.platformTelemetryFunctional).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.platformTelemetryRestartSurvivalClaimed).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.metricsCollectionImplemented).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.exportersImplemented).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.dashboardsImplemented).toBe(false);
    expect(W5_N15_B_ARCHITECTURE_CLAIMS.runtimeAggregationImplemented).toBe(false);
  ...
  it('explicit OUT covers telemetry runtime only (not operational continuity)', () => {
    expect(W5_N15_B_EXPLICIT_OUT).toEqual(
      expect.arrayContaining([
    ...
    expect(W5_N15_B_EXPLICIT_OUT).not.toContain('w5-n14-d');
  });
  ...
  it('schema and module wiring evidence exist', () => {
    expect(
      existsSync(
        join(
          REPO_ROOT,
          'apps/api/src/modules/notification-delivery/domain/durable-notification-platform-telemetry-anchor.ts',
        ),
      ),
    ).toBe(true);
    expect(
  ...
  it('required reports exist for W5-N15-b', () => {
