...
import {
  W5_N13_A_ALLOWED_OWNERS,
  W5_N13_A_ARCHITECTURE_CLAIMS,
  W5_N13_A_ARTIFACT_KINDS,
  W5_N13_A_BINDING_FINDINGS,
  W5_N13_A_CAPABILITY_CATEGORIES,
  W5_N13_A_DURABILITY_CLASSES,
  W5_N13_A_EXPLICIT_OUT,
  W5_N13_A_HONEST_PRODUCT_BASELINE,
  W5_N13_A_REQUIRED_ARTIFACT_KINDS,
  W5_N13_A_SLICE_ID,
  ...
  rowsNotificationPlatformRetryEphemeral,
  rowsNotificationPlatformRetrySurvive,
  rowsSurvive,
} from './w5-n13-a-notification-platform-retry-inventory';

const REPO_ROOT = join(__dirname, '../../../..');

describe('W5-N13-a notification platform retry inventory — unit', () => {
  it('inventory completeness: every required artifact kind appears', () => {
    const kinds = new Set(W5_N13_A_NOTIFICATION_PLATFORM_RETRY_INVENTORY.map((row) => row.kind));
    for (const kind of W5_N13_A_REQUIRED_ARTIFACT_KINDS) {
      expect(kinds.has(kind)).toBe(true);
    }
    expect(W5_N13_A_ARTIFACT_KINDS).toEqual([...W5_N13_A_REQUIRED_ARTIFACT_KINDS]);
  });
  ...
  it('inventory completeness: every row has required classification and responsibility fields', () => {
    for (const row of W5_N13_A_NOTIFICATION_PLATFORM_RETRY_INVENTORY) {
      expect(W5_N13_A_ALLOWED_OWNERS).toContain(row.owner);
      expect(W5_N13_A_DURABILITY_CLASSES).toContain(row.durabilityClass);
      expect(W5_N13_A_CAPABILITY_CATEGORIES).toContain(row.capabilityCategory);
      expect(row.currentStatus.length).toBeGreaterThan(0);
      expect(row.honestyRequirement.length).toBeGreaterThan(10);
      expect(row.futureW5N13Responsibility.length).toBeGreaterThan(0);
      ...
      expect(row.authorizesPlatformRetryFunctional).toBe(false);
      expect(row.authorizesW5N13Complete).toBe(false);
    }
  });
  ...
  it('ownership consistency: core ownership rows stay on notification substrate owners', () => {
    ...
    for (const row of ownership.filter((entry) => coreIds.includes(entry.artifactId))) {
      expect(W5_N13_A_SUBSTRATE_OWNERS).toContain(row.owner);
    }
    expect(
  ...
  it('platform coverage: per-channel anchors, W5-N05/N06 foundation consumption, PC-06 routing, missing queue layer', () => {
    ...
    expect(ids.has('missing-unified-platform-retry-view')).toBe(true);
    expect(ids.has('missing-platform-retry-restart-recovery')).toBe(true);
    expect(ids.has('missing-platform-retry-operational-continuity')).toBe(true);
  ...
  it('distinction consistency: workers foundation / platform ready / per-channel honesty boundaries', () => {
    ...
    expect(ids.has('honesty-platform-retry-not-live-trading')).toBe(true);
    expect(ids.has('honesty-platform-ready-requires-retry-evidence')).toBe(true);
    expect(ids.has('honesty-foundation-not-transport-i/o')).toBe(true);
    expect(ids.has('honesty-scheduler-foundation-not-retry-complete')).toBe(true);
  ...
  it('honesty blockers: missing unified delivery layer, recovery, continuity, anchors, production transport', () => {
    ...
    expect(ids.has('consume-w5-n07-dispatch-anchor')).toBe(true);
    expect(ids.has('consume-w5-n11-worker-runtime-anchor')).toBe(true);
    expect(ids.has('persist-notification-platform-retry-anchor')).toBe(true);
    expect(ids.has('missing-platform-retry-ui')).toBe(true);
  });

  it('honesty: no row authorizes platform retry functional; queue does not function from slice a', () => {
    expect(W5_N13_A_BINDING_FINDINGS.platformRetryFunctionalAuthorized).toBe(false);
    expect(W5_N13_A_BINDING_FINDINGS.platformRetryFunctionsAfterSliceA).toBe(false);
    expect(W5_N13_A_BINDING_FINDINGS.customerVisibleFeatureFromSliceA).toBe(false);
  ...
  it('classification: SURVIVE vs EPHEMERAL partitions are non-empty and exclusive', () => {
    ...
    expect(survive.length + ephemeral.length).toBe(
      W5_N13_A_NOTIFICATION_PLATFORM_RETRY_INVENTORY.length,
    );
  });

  it('platform retry SURVIVE/EPHEMERAL subsets are documented', () => {
    expect(rowsNotificationPlatformRetrySurvive().length).toBeGreaterThan(0);
    expect(rowsNotificationPlatformRetryEphemeral().length).toBeGreaterThan(0);
  });

  it('capability categories cover implemented, infrastructure, planned, not-implemented, future-roadmap', () => {
    for (const category of W5_N13_A_CAPABILITY_CATEGORIES) {
      expect(rowsByCapabilityCategory(category).length).toBeGreaterThan(0);
    }
  });

  it('explicit OUT surfaces cover platform retry impl, b–e, scheduler/scheduler/retry, Live Trading, W5-N05 reopen', () => {
    ...
    expect(ids.has('out-w5-n13-b-durable-scheduler')).toBe(true);
    expect(ids.has('out-w5-n13-c-restart-recovery')).toBe(true);
    expect(ids.has('out-w5-n13-d-operational-continuity')).toBe(true);
    expect(ids.has('out-w5-n13-e-close-evidence')).toBe(true);
  ...
  it('W5-N11 worker runtime foundation consumed not reopened', () => {
    expect(W5_N13_A_ALLOWED_OWNERS).toContain('w5-n10-reference');
    const ids = new Set(artifactIds());
    expect(ids.has('consume-w5-n11-worker-runtime-anchor')).toBe(true);
    expect(ids.has('own-w5-n11-worker-runtime-foundation-consume')).toBe(true);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.w5N10Reopened).toBe(false);
  });

  it('W5-N11 worker runtime foundation consumed not reopened', () => {
    expect(W5_N13_A_ALLOWED_OWNERS).toContain('w5-n06-reference');
    const ids = new Set(artifactIds());
    expect(ids.has('consume-w5-n06-delivery-anchor')).toBe(true);
    expect(ids.has('consume-w5-n06-delivery-continuity-view')).toBe(true);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.w5N11Reopened).toBe(false);
  });

  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    expect(W5_N13_A_HONEST_PRODUCT_BASELINE.implementedCapabilities[0]).toContain('None');
    expect(
      W5_N13_A_HONEST_PRODUCT_BASELINE.infrastructureCapabilities.length,
    ).toBeGreaterThanOrEqual(8);
    expect(W5_N13_A_HONEST_PRODUCT_BASELINE.plannedCapabilities.length).toBeGreaterThanOrEqual(1);
    expect(
      W5_N13_A_HONEST_PRODUCT_BASELINE.notYetImplementedCapabilities.length,
    ).toBeGreaterThanOrEqual(5);
    expect(
      W5_N13_A_HONEST_PRODUCT_BASELINE.futureRoadmapCapabilities.length,
  ...
  it('technical debt delta: inventory resolved; b–e deferred; nothing introduced', () => {
    ...
    expect(W5_N13_A_TECHNICAL_DEBT_DELTA.deferred.length).toBe(1);
  });
});

describe('W5-N13-a notification platform retry inventory — integration / planning', () => {
  it('planning consistency: slice id and architecture claims remain inventory-only', () => {
    expect(W5_N13_A_SLICE_ID).toBe('W5-N13-a');
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.newBoundedContext).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.duplicateNotificationSubsystem).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.duplicateRoutingEngine).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.exchangeAdapterUntouched).toBe(true);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.notificationControlPlane).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.customerVisibleFeature).toBe(false);
  ...
  it('master plan consistency: inventory claims leave Master Plan / V2 / Wave 1–4 unchanged', () => {
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.masterPlanModified).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.version2Redesigned).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.wave1Modified).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.wave2Modified).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.wave3Modified).toBe(false);
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.wave4Modified).toBe(false);
  ...
  it('architecture consistency: no duplicate notification subsystem / persistence owner / routing engine', () => {
    expect(W5_N13_A_ARCHITECTURE_CLAIMS.newSourceOfTruth).toBe(false);
    for (const item of [
      ...
      expect(W5_N13_A_EXPLICIT_OUT).toContain(item);
    }
  });
