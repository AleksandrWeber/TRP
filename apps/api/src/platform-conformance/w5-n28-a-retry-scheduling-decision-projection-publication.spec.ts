...
import {
  W5_N28_A_CONFORMANCE_SLICE_ID,
  W5_N28_A_REQUIRED_REPORTS,
...
} from './w5-n28-a-retry-scheduling-decision-projection-publication';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N28-a retry scheduling decision projection publication conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N28_A_CONFORMANCE_SLICE_ID).toBe('W5-N28-a');
    expect(W5_N28_A_REQUIRED_REPORTS.length).toBe(6);
    expect(W5_N28_A_REQUIRED_REPORTS).toEqual([
    ...
    expect(W5_N28_A_DECISION_CLASSIFICATIONS).toEqual([
      'DECISION',
      'CONFIGURATION',
      'EPHEMERAL',
      'RECOVERABLE',
  ...
  it('inventory completeness verifies required ownership rows and no scheduling authorization', () => {
    ...
    expect(inventory.noRowAuthorizesPublicationFunctional).toBe(true);
    expect(inventory.noRowAuthorizesW5N28Complete).toBe(true);
  ...
  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    ...
    expect(honest.publicationNotAuthorized).toBe(true);
  ...
  it('architecture integrity: ownership preserved; no Runtime Scheduler; Exchange Adapter untouched', () => {
    ...
    expect(W5_N28_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('honesty boundaries: inventory-only honesty rules frozen', () => {
    ...
    expect(honesty.inventoryOnlyNotRuntimeDecisionProjectionPublication).toBe(true);
  ...
  it('binding findings: Scheduling cannot function after slice a', () => {
    ...
    expect(W5_N28_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N28_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
...
describe('W5-N28-a retry scheduling decision projection publication conformance — integration / planning', () => {
  it('required reports list is frozen (docs created by parent / package close)', () => {
    ...
    const inventoryReport = join(WAVE5, 'w5-n28-a-inventory.md');
    if (existsSync(inventoryReport)) {
      for (const name of W5_N28_A_REQUIRED_REPORTS) {
  ...
  it('conformance source files exist on disk', () => {
    ...
    expect(
      existsSync(
        join(
          REPO_ROOT,
          'apps/api/src/platform-conformance/w5-n28-a-retry-scheduling-decision-projection-publication.ts',
        ),
      ),
    ).toBe(true);
  });
