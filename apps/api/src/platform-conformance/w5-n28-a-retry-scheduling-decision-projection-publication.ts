/**
 * W5-N28-a — Notification Retry Scheduling Decision Projection Publication Conformance Registry.
 *
 * Validates the canonical platform decision projection publication inventory and Honest Product baseline.
...
export const W5_N28_A_CONFORMANCE_SLICE_ID = W5_N28_A_SLICE_ID;

export const W5_N28_A_REQUIRED_REPORTS = Object.freeze([
  'w5-n28-a-inventory.md',
...
export function verifyInventoryCompleteness(): Readonly<{
  ok: boolean;
  rowCount: number;
  noRowAuthorizesPublicationFunctional: boolean;
  noRowAuthorizesW5N28Complete: boolean;
  ...
  const noRowAuthorizesPublicationFunctional =
    W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.every(
      (entry) => !entry.authorizesPublicationFunctional,
    );
  const noRowAuthorizesW5N28Complete =
    W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.every(
      (entry) => !entry.authorizesW5N28Complete,
    );
  return Object.freeze({
    ok:
      W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.length >= 50 &&
      noRowAuthorizesPublicationFunctional &&
      noRowAuthorizesW5N28Complete &&
      requiredOwnershipRowsPresent,
    rowCount: W5_N28_A_RETRY_SCHEDULING_DECISION_PROJECTION_PUBLICATION_INVENTORY.length,
    noRowAuthorizesPublicationFunctional,
    noRowAuthorizesW5N28Complete,
    requiredOwnershipRowsPresent,
  });
...
export function verifyHonestProductBaseline(): Readonly<{
  ...
  publicationNotAuthorized: boolean;
  deliveryOnlyNotControlPlane: boolean;
}> {
  const implemented = W5_N28_A_HONEST_PRODUCT_BASELINE.implementedCapabilities;
  const noCustomerVisibleImplemented =
    implemented.length === 1 && implemented[0]?.includes('None') === true;
  const infrastructureDocumented =
    W5_N28_A_HONEST_PRODUCT_BASELINE.infrastructureCapabilities.length >= 8;
  const plannedExplicit = W5_N28_A_HONEST_PRODUCT_BASELINE.plannedCapabilities.length >= 1;
  const notImplementedExplicit =
    W5_N28_A_HONEST_PRODUCT_BASELINE.notYetImplementedCapabilities.length >= 5;
  const publicationNotAuthorized = !W5_N28_A_BINDING_FINDINGS.publicationFunctionalAuthorized;
  const deliveryOnlyNotControlPlane = !W5_N28_A_ARCHITECTURE_CLAIMS.notificationControlPlane;
  return Object.freeze({
    ok:
...
export function verifyArchitectureIntegrity(): Readonly<{
  ...
  const ownershipUnchanged = !W5_N28_A_ARCHITECTURE_CLAIMS.ownershipBoundariesChanged;
  const noDuplicateSubsystem =
    !W5_N28_A_ARCHITECTURE_CLAIMS.duplicateNotificationSubsystem &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.duplicateDecisionProjectionPublicationSubsystem &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.duplicateRetrySubsystem;
  const noRuntimeScheduler =
    !W5_N28_A_ARCHITECTURE_CLAIMS.runtimeSchedulerIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.retryEngineIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.schedulerPlatformIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.workflowEngineIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.eventBusProductIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.orchestrationPlatformIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.workerIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.timerImplementationIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.runtimeDecisionProjectionPublicationIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.runtimePublicationEngineIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.runtimeDecisionEngineIntroduced &&
    !W5_N28_A_ARCHITECTURE_CLAIMS.runtimeSchedulingIntroduced;
  const noMasterPlanChange = !W5_N28_A_ARCHITECTURE_CLAIMS.masterPlanModified;
  const exchangeAdapterUntouched = W5_N28_A_ARCHITECTURE_CLAIMS.exchangeAdapterUntouched;
  const notificationControlPlaneForbidden = !W5_N28_A_ARCHITECTURE_CLAIMS.notificationControlPlane;
  return Object.freeze({
    ok:
      ownershipUnchanged &&
...
export function verifyOwnershipBoundaries(): Readonly<{
  ...
  const substrateOwnersFrozen = coreOwnership.every((entry) =>
    (W5_N28_A_SUBSTRATE_OWNERS as readonly string[]).includes(entry.owner),
  );
  return Object.freeze({
    ok:
      W5_N28_A_BINDING_FINDINGS.ownershipBoundariesVerified &&
      !W5_N28_A_BINDING_FINDINGS.ownershipBoundariesChanged &&
      !W5_N28_A_ARCHITECTURE_CLAIMS.newPersistenceOwner &&
      substrateOwnersFrozen,
    substrateOwnersFrozen,
    ownershipVerified: W5_N28_A_BINDING_FINDINGS.ownershipBoundariesVerified,
    newPersistenceOwner: W5_N28_A_ARCHITECTURE_CLAIMS.newPersistenceOwner,
  });
}
...
export function buildPublicationDiagnostics(): Readonly<{
  ...
  technicalDebtDelta: typeof W5_N28_A_TECHNICAL_DEBT_DELTA;
  ok: boolean;
}> {
  ...
  return Object.freeze({
    ...
    ownership,
    honesty,
    technicalDebtDelta: W5_N28_A_TECHNICAL_DEBT_DELTA,
    ok:
      inventory.ok &&
      honestProduct.ok &&
      architecture.ok &&
      ownership.ok &&
      honesty.ok &&
      !W5_N28_A_BINDING_FINDINGS.publicationFunctionsAfterSliceA,
  });
}
