...
import { W5_N04_A_ARCHITECTURE_CLAIMS } from './w5-n04-a-push-notification-inventory';
...
import {
  ...
  W5_N04_E_APPROVED_SLICES,
  W5_N04_E_ARCHITECTURE_CLAIMS,
  W5_N04_E_BINDING_FINDINGS,
  W5_N04_E_CAPABILITY_EVOLUTION,
  W5_N04_E_INTEGRITY_NON_EXPANSION,
  W5_N04_E_NOTIFICATION_OWNER,
  W5_N04_E_OPERATIONAL_MATURITY,
  W5_N04_E_REQUIRED_REPORTS,
  W5_N04_E_REQUIRED_SLICE_REPORTS,
  W5_N04_E_SLICE_ID,
  W5_N04_E_TECHNICAL_DEBT_DELTA,
  W5_N04_E_TRANSITION_MATRIX,
} from './w5-n04-e-package-close-evidence';
...
describe('W5-N04-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N04_E_SLICE_ID).toBe('W5-N04-e');
    expect(W5_N04_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N04_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N04-a',
      'W5-N04-b',
      'W5-N04-c',
      'W5-N04-d',
    ...
    expect(
      W5_N04_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N04_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });

  it('Honest Product enforcement intact', () => {
    const honest = verifyHonestProduct();
    expect(honest.ok).toBe(true);
    expect(honest.operationalContinuityNotPushTransport).toBe(true);
    ...
    expect(W5_N04_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N04_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N04_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, Push Notification Complete, or Wave 5 COMPLETE', () => {
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.w5N04CompleteClaimed).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N04_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
  });
});

describe('W5-N04-e package close evidence — integration / planning', () => {
  it('transition safety answers hold', () => {
    ...
    expect(answers.w5N04CompleteNotClaimed).toBe(true);
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.webPushTransport).toBe(false);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.liveTrading).toBe(false);
    expect(W5_N04_E_INTEGRITY_NON_EXPANSION).toContain('Web Push Transport');
    expect(W5_N04_E_INTEGRITY_NON_EXPANSION).toContain(
      'Final Package Integration Verification Performed',
    );
  });

  it('product verification claims hold across c/d', () => {
    expect(W5_N04_C_ARCHITECTURE_CLAIMS.recoveryDeterministic).toBe(true);
    expect(W5_N04_C_ARCHITECTURE_CLAIMS.recoveryIdempotent).toBe(true);
    expect(W5_N04_C_ARCHITECTURE_CLAIMS.pushNotificationAnchorStateRestoredAfterRestart).toBe(true);
    expect(W5_N04_D_ARCHITECTURE_CLAIMS.operationalContinuityDerived).toBe(true);
  ...
  it('required close evidence reports exist', () => {
    for (const name of W5_N04_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N04_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n04-a-push-notification-inventory.md'))).toBe(true);
    ...
    expect(existsSync(join(WAVE5, 'w5-n04-product-owner-close-record.md'))).toBe(false);
  });

  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N04_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N04_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N04_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N04_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N04_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N04_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/without Web Push/);
    expect(W5_N04_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N04_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
  });

  it('platform readiness exposes push notification continuity without Connected / Web Push controls', () => {
    ...
    expect(api).toContain('PushNotificationContinuityView');
    expect(view).toMatch(/Push Notification|pushNotification/i);
  ...
  it('status docs: W5-N04 slices complete; package not CLOSED; Wave 5 COMPLETE not claimed', () => {
    const progress = readWave5('wave-5-progress.md');
    const overview = readWave5('wave-5-overview.md');
    const close = readWave5('w5-n04-package-close-report.md');
    const summary = readWave5('w5-n04-package-summary.md');
    expect(progress).toMatch(/W5-N04-a|W5-N04-b|W5-N04-c|W5-N04-d|W5-N04-e/);
    expect(progress).toMatch(/Awaiting Final Package Integration Verification|W5-N04-e/);
    expect(progress).toMatch(/Wave 5 COMPLETE[\s\S]*Not claimed|Not claimed[\s\S]*Wave 5 COMPLETE/);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /Push notifications operational[\s\S]*not claimed|not claimed[\s\S]*Push notifications operational/i,
    );
    expect(close).toMatch(/Awaiting Final Package Integration Verification|not performed/i);
    expect(close).toMatch(/Wave 5 COMPLETE[\s\S]*not claimed|not claimed[\s\S]*Wave 5 COMPLETE/i);
    expect(summary).toMatch(/not authorized|not operational|foundation scope only/i);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N04_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
  });
  ...
  it('slice validation reports a–d exist and record PASS', () => {
    for (const slice of ['a', 'b', 'c', 'd'] as const) {
      const report = readWave5(`w5-n04-${slice}-validation-report.md`);
