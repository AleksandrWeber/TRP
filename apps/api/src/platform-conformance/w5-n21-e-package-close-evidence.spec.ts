...
import { W5_N21_A_ARCHITECTURE_CLAIMS } from './w5-n21-a-retry-backoff-inventory';
...
import {
  ...
  verifyRetryBackoffFoundationChain,
  W5_N21_E_APPROVED_SLICES,
  W5_N21_E_ARCHITECTURE_CLAIMS,
  W5_N21_E_BINDING_FINDINGS,
  W5_N21_E_CAPABILITY_EVOLUTION,
  W5_N21_E_DEPENDENCY_CHAIN,
  W5_N21_E_INTEGRITY_NON_EXPANSION,
  W5_N21_E_NOTIFICATION_OWNER,
  W5_N21_E_OPERATIONAL_MATURITY,
  W5_N21_E_REQUIRED_REPORTS,
  W5_N21_E_REQUIRED_SLICE_REPORTS,
  W5_N21_E_SLICE_ID,
  W5_N21_E_TECHNICAL_DEBT_DELTA,
  W5_N21_E_TRANSITION_MATRIX,
} from './w5-n21-e-package-close-evidence';
...
describe('W5-N21-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N21_E_SLICE_ID).toBe('W5-N21-e');
    expect(W5_N21_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N21_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N21-a',
      'W5-N21-b',
      'W5-N21-c',
      'W5-N21-d',
    ...
    expect(
      W5_N21_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('implementation chain complete', () => {
    ...
    expect(W5_N21_E_BINDING_FINDINGS.implementationChainComplete).toBe(true);
  });
  ...
  it('dependency chain intact — prior packages closed and consumed, not reopened', () => {
    ...
    expect(W5_N21_E_DEPENDENCY_CHAIN.map((link) => link.packageId)).toEqual([
      'W5-N01',
      'W5-N02',
      'W5-N03',
      'W5-N04',
    ...
    expect(W5_N21_E_BINDING_FINDINGS.dependencyChainIntact).toBe(true);
  });

  it('retry backoff foundation chain integrity holds', () => {
    const foundation = verifyRetryBackoffFoundationChain();
    ...
    expect(W5_N21_E_BINDING_FINDINGS.retryBackoffFoundationChainIntact).toBe(true);
  });
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N21_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });
  ...
  it('Honest Product enforcement intact', () => {
    ...
    expect(honest.retryBackoffFunctionalNotAuthorized).toBe(true);
    expect(honest.retryBackoffNotClaimed).toBe(true);
    expect(W5_N21_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N21_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N21_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, retry backoff functional, or Wave 5 COMPLETE', () => {
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.w5N21CompleteClaimed).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.retryBackoffFunctionalClaimed).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N21_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
    expect(diagnostics.retryBackoffFoundation.ok).toBe(true);
  });
});

describe('W5-N21-e package close evidence — integration / planning', () => {
  it('transition safety answers hold', () => {
    ...
    expect(answers.w5N21CompleteNotClaimed).toBe(true);
    expect(answers.notificationPlatformCompleteNotClaimed).toBe(true);
    expect(answers.retryBackoffFunctionalNotClaimed).toBe(true);
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.retryBackoffImplemented).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.backoffCalculationImplemented).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.exponentialBackoffImplemented).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.linearBackoffImplemented).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.policyEvaluationRuntime).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.deliveryExecutionRuntime).toBe(false);
    expect(W5_N21_E_ARCHITECTURE_CLAIMS.liveTrading).toBe(false);
    expect(W5_N21_E_INTEGRITY_NON_EXPANSION).toContain('Retry Backoff Runtime');
    expect(W5_N21_E_INTEGRITY_NON_EXPANSION).toContain('Notification Platform COMPLETE');
    expect(W5_N21_E_INTEGRITY_NON_EXPANSION).toContain(
      'Final Package Integration Verification Performed',
    );
  });

  it('product verification claims hold across c/d', () => {
    expect(W5_N21_C_ARCHITECTURE_CLAIMS.recoveryDeterministic).toBe(true);
    expect(W5_N21_C_ARCHITECTURE_CLAIMS.recoveryIdempotent).toBe(true);
    expect(
  ...
  it('required close evidence reports exist', () => {
    for (const name of W5_N21_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N21_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n21-a-retry-backoff-inventory.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'notification-retry-backoff-overview.md'))).toBe(true);
    ...
    expect(existsSync(join(WAVE5, 'w5-n21-planning-summary.md'))).toBe(true);
  });

  it('status docs: W5-N21 CLOSED by Product Owner; Wave 5 COMPLETE not claimed', () => {
    const progress = readWave5('wave-5-progress.md');
    const overview = readWave5('notification-retry-backoff-overview.md');
    const close = readWave5('w5-n21-close-package-report.md');
    const summary = readWave5('w5-n21-package-summary.md');
    const finalIntegration = readWave5('w5-n21-final-integration-verification.md');
    const poClose = readWave5('w5-n21-product-owner-close-record.md');
    expect(existsSync(join(WAVE5, 'w5-n21-final-integration-verification.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n21-product-owner-close-record.md'))).toBe(true);
    expect(progress).toMatch(/W5-N21-a|W5-N21-b|W5-N21-c|W5-N21-d|W5-N21-e/);
    expect(progress).toMatch(/CLOSED by Product Owner|W5-N21\s+\*\*CLOSED\*\*/i);
    ...
    expect(overview).toMatch(/CLOSED by Product Owner|W5-N21[\s\S]*CLOSED/i);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /Retry Backoff implemented[\s\S]*not|not[\s\S]*Retry Backoff implemented|Do not declare Retry Backoff implemented|Do NOT declare Retry Backoff implemented/i,
    );
  ...
  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N21_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N21_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N21_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N21_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N21_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N21_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(
      /without retry backoff runtime/,
    );
    expect(W5_N21_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N21_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W5_N21_E_TECHNICAL_DEBT_DELTA.deferred).toContain(
      'Final Package Integration Verification',
    );
  });

  it('platform readiness exposes notification platform retry backoff continuity without implementation controls', () => {
    ...
    expect(api).toContain('NotificationPlatformRetryBackoffContinuityView');
    expect(view).toMatch(/Notification Platform Retry Backoff|notificationPlatformRetryBackoff/i);
    expect(readiness).toMatch(/notificationPlatformRetryBackoff/);
  ...
  it('slice validation reports a–d exist and record PASS', () => {
    for (const slice of ['a', 'b', 'c', 'd'] as const) {
      const report = readWave5(`w5-n21-${slice}-validation-report.md`);
