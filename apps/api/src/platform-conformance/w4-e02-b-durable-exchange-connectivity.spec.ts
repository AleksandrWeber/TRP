...
import {
  ...
  W4_E02_B_DURABLE_COVERAGE,
  W4_E02_B_EXCHANGE_CONNECTIVITY_OWNER,
  W4_E02_B_EXPLICIT_OUT,
  W4_E02_B_NEW_PERSISTED_ARTIFACT_IDS,
  W4_E02_B_PREEXISTING_SURVIVE_ARTIFACT_IDS,
  W4_E02_B_SLICE_ID,
  W4_E02_B_TECHNICAL_DEBT_DELTA,
  W4_E02_B_TRANSITION_MATRIX,
  newPersistedInventoryRows,
  persistedArtifactIds,
  preexistingSurviveInventoryRows,
} from './w4-e02-b-durable-exchange-connectivity';
...
describe('W4-E02-b durable Bybit exchange connectivity — unit', () => {
  ...
  it('artifact coverage: only approved new SURVIVE row is persisted by this slice', () => {
    expect([...persistedArtifactIds()].sort()).toEqual(
      [...W4_E02_B_NEW_PERSISTED_ARTIFACT_IDS].sort(),
    );
    const rows = newPersistedInventoryRows();
    expect(rows.length).toBe(W4_E02_B_NEW_PERSISTED_ARTIFACT_IDS.length);
    for (const row of rows) {
      expect(row.artifactId).toBe('persist-bybit-connection-continuity');
  ...
  it('pre-existing SURVIVE rows remain on vault / connection-management / exchange-adapter owners', () => {
    ...
    expect(preexisting.length).toBe(W4_E02_B_PREEXISTING_SURVIVE_ARTIFACT_IDS.length);
  ...
  it('ownership: Bybit exchange connectivity persistence remains on exchange-adapter owner only', () => {
    expect(W4_E02_B_EXCHANGE_CONNECTIVITY_OWNER).toBe('exchange-adapter');
    for (const row of W4_E02_B_DURABLE_COVERAGE) {
  ...
  it('transition matrix: inventory → durable persistence → restart recovery still missing', () => {
    expect(W4_E02_B_TRANSITION_MATRIX.before).toContain('Inventory');
    expect(W4_E02_B_TRANSITION_MATRIX.after).toContain('Durable Persistence');
    expect(W4_E02_B_TRANSITION_MATRIX.stillMissing.some((item) => item.includes('Restart'))).toBe(
      true,
    );
  });
});

describe('W4-E02-b durable Bybit exchange connectivity — integration', () => {
  it('persistence lifecycle: no recovery / I/O / continuity claims from this slice', () => {
    expect(W4_E02_B_SLICE_ID).toBe('W4-E02-b');
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.automaticRestartRecovery).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.operationalContinuityGuaranteed).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.restImplementation).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.websocketImplementation).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.exchangeIoEstablished).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.exchangeConnectivityRestartSurvivalClaimed).toBe(false);
    expect(W4_E02_B_ARCHITECTURE_CLAIMS.w4E01Reopened).toBe(false);
  });

  it('technical debt delta: persistence foundation resolved; recovery deferred', () => {
    expect(W4_E02_B_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W4_E02_B_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W4_E02_B_TECHNICAL_DEBT_DELTA.deferred.some((item) => item.includes('restart'))).toBe(
      true,
    );
  });

  it('explicit OUT covers restart recovery and W4-E02-c', () => {
    expect(W4_E02_B_EXPLICIT_OUT).toEqual(
      expect.arrayContaining(['restart-recovery', 'w4-e02-c', 'rest-connectivity']),
    );
  });
  ...
  it('SURVIVE inventory includes rows beyond new persistence — not all are W4-E02-b targets', () => {
    expect(rowsExchangeConnectivitySurvive().length).toBeGreaterThan(
      W4_E02_B_NEW_PERSISTED_ARTIFACT_IDS.length,
    );
  });

  it('required reports exist for W4-E02-b', () => {
