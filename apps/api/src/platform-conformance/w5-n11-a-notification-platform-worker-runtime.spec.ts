...
import {
  W5_N11_A_CONFORMANCE_SLICE_ID,
  W5_N11_A_REQUIRED_REPORTS,
  buildNotificationPlatformWorkerRuntimeDiagnostics,
...
} from './w5-n11-a-notification-platform-worker-runtime';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N11-a notification platform workers conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N11_A_CONFORMANCE_SLICE_ID).toBe('W5-N11-a');
    expect(W5_N11_A_REQUIRED_REPORTS.length).toBe(6);
  });
  ...
  it('inventory completeness verifies required ownership rows and no platform workers authorization', () => {
    ...
    expect(inventory.noRowAuthorizesPlatformWorkerExecutionFunctional).toBe(true);
    expect(inventory.noRowAuthorizesW5N10Complete).toBe(true);
  ...
  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    ...
    expect(honest.platformWorkerExecutionNotAuthorized).toBe(true);
    expect(honest.workerExecutionOnlyNotControlPlane).toBe(true);
  });
  ...
  it('architecture integrity: ownership preserved; no duplicate subsystem; Exchange Adapter untouched', () => {
    ...
    expect(W5_N11_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('honesty boundaries: workers foundation, platform ready, and per-channel rules frozen', () => {
    ...
    expect(honesty.workerExecutionFoundationNotLiveTrading).toBe(true);
    expect(honesty.platformReadyRequiresEvidence).toBe(true);
    expect(honesty.workersNotWorkerExecutionComplete).toBe(true);
  });
  ...
  it('binding findings: Notification Platform Workers cannot function after slice a', () => {
    ...
    expect(W5_N11_A_ARCHITECTURE_CLAIMS.platformWorkerRuntimeFunctionalClaimed).toBe(false);
    expect(W5_N11_A_ARCHITECTURE_CLAIMS.w5N11CompleteClaimed).toBe(false);
    expect(W5_N11_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N11_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
  ...
  it('diagnostics roll-up passes for slice a scope', () => {
    const diagnostics = buildNotificationPlatformWorkerRuntimeDiagnostics();
...
describe('W5-N11-a notification platform workers conformance — integration / planning', () => {
  it('required reports exist for W5-N11-a', () => {
    for (const name of W5_N11_A_REQUIRED_REPORTS) {
