...
import { W5_N13_A_ARCHITECTURE_CLAIMS } from './w5-n13-a-notification-platform-retry-inventory';
...
import {
  ...
  verifyRetryFoundationChain,
  W5_N13_E_APPROVED_SLICES,
  W5_N13_E_ARCHITECTURE_CLAIMS,
  W5_N13_E_BINDING_FINDINGS,
  W5_N13_E_CAPABILITY_EVOLUTION,
  W5_N13_E_DEPENDENCY_CHAIN,
  W5_N13_E_INTEGRITY_NON_EXPANSION,
  W5_N13_E_NOTIFICATION_OWNER,
  W5_N13_E_OPERATIONAL_MATURITY,
  W5_N13_E_REQUIRED_REPORTS,
  W5_N13_E_REQUIRED_SLICE_REPORTS,
  W5_N13_E_SLICE_ID,
  W5_N13_E_TECHNICAL_DEBT_DELTA,
  W5_N13_E_TRANSITION_MATRIX,
} from './w5-n13-e-package-close-evidence';
...
describe('W5-N13-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N13_E_SLICE_ID).toBe('W5-N13-e');
    expect(W5_N13_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N13_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N13-a',
      'W5-N13-b',
      'W5-N13-c',
      'W5-N13-d',
    ...
    expect(
      W5_N13_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('implementation chain complete', () => {
    ...
    expect(W5_N13_E_BINDING_FINDINGS.implementationChainComplete).toBe(true);
  });
  ...
  it('dependency chain intact — prior packages closed and consumed, not reopened', () => {
    ...
    expect(W5_N13_E_DEPENDENCY_CHAIN.map((link) => link.packageId)).toEqual([
      'W5-N01',
      'W5-N02',
      'W5-N03',
      'W5-N04',
    ...
    expect(W5_N13_E_BINDING_FINDINGS.dependencyChainIntact).toBe(true);
  });

  it('retry foundation chain integrity holds', () => {
    const foundation = verifyRetryFoundationChain();
    ...
    expect(W5_N13_E_BINDING_FINDINGS.retryFoundationChainIntact).toBe(true);
  });
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N13_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });

  it('Honest Product enforcement intact', () => {
    const honest = verifyHonestProduct();
    expect(honest.ok).toBe(true);
    expect(honest.operationalContinuityNotPlatformRetryRuntime).toBe(true);
    expect(honest.restartRecoveryNotProductionReady).toBe(true);
    expect(honest.inventoryHonestBaselineIntact).toBe(true);
    expect(honest.platformRetryFunctionalNotAuthorized).toBe(true);
    expect(honest.retryRuntimeNotClaimed).toBe(true);
    expect(W5_N13_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N13_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N13_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, retry functional, or Wave 5 COMPLETE', () => {
    ...
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.w5N13CompleteClaimed).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.platformRetryFunctionalClaimed).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N13_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
  });
});

describe('W5-N13-e package close evidence — integration / planning', () => {
  it('transition safety answers hold', () => {
    ...
    expect(answers.w5N13CompleteNotClaimed).toBe(true);
    expect(answers.notificationPlatformCompleteNotClaimed).toBe(true);
    expect(answers.platformRetryFunctionalNotClaimed).toBe(true);
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.retryRuntimeImplemented).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.retryExecutionImplemented).toBe(false);
    expect(W5_N13_E_ARCHITECTURE_CLAIMS.liveTrading).toBe(false);
    expect(W5_N13_E_INTEGRITY_NON_EXPANSION).toContain('Retry Runtime Implementation');
    expect(W5_N13_E_INTEGRITY_NON_EXPANSION).toContain(
      'Final Package Integration Verification Performed',
    );
  });

  it('product verification claims hold across c/d', () => {
    expect(W5_N13_C_ARCHITECTURE_CLAIMS.recoveryDeterministic).toBe(true);
    expect(W5_N13_C_ARCHITECTURE_CLAIMS.recoveryIdempotent).toBe(true);
    expect(
      W5_N13_C_ARCHITECTURE_CLAIMS.notificationPlatformRetryAnchorStateRestoredAfterRestart,
    ).toBe(true);
    expect(W5_N13_D_ARCHITECTURE_CLAIMS.operationalContinuityDerived).toBe(true);
  ...
  it('required close evidence reports exist', () => {
    for (const name of W5_N13_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N13_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n13-a-notification-platform-retry-inventory.md'))).toBe(true);
    ...
    expect(existsSync(join(WAVE5, 'w5-n13-final-integration-verification.md'))).toBe(false);
  ...
  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N13_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N13_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N13_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N13_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N13_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N13_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/without retry runtime/);
    expect(W5_N13_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N13_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W5_N13_E_TECHNICAL_DEBT_DELTA.deferred).toContain(
      'Final Package Integration Verification',
    );
  });

  it('platform readiness exposes notification platform retry continuity without implementation controls', () => {
    ...
    expect(api).toContain('NotificationPlatformRetryContinuityView');
    expect(view).toMatch(/Notification Platform Retry|notificationPlatformRetry/i);
  ...
  it('slice validation reports a–d exist and record PASS', () => {
    for (const slice of ['a', 'b', 'c', 'd'] as const) {
      const report = readWave5(`w5-n13-${slice}-validation-report.md`);
