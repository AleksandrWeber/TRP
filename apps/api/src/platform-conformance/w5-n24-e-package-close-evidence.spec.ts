...
import { W5_N24_A_ARCHITECTURE_CLAIMS } from './w5-n24-a-retry-scheduling-inventory';
...
import {
  ...
  verifySchedulingFoundationChain,
  verifyGovernanceIntegrity,
  verifyHonestProduct,
  verifyImplementationChain,
  verifyOperationalChain,
  W5_N24_E_APPROVED_SLICES,
  W5_N24_E_ARCHITECTURE_CLAIMS,
  W5_N24_E_BINDING_FINDINGS,
  W5_N24_E_CAPABILITY_EVOLUTION,
  W5_N24_E_DEPENDENCY_CHAIN,
  W5_N24_E_NOTIFICATION_OWNER,
  W5_N24_E_OPERATIONAL_MATURITY,
  W5_N24_E_REQUIRED_REPORTS,
  W5_N24_E_REQUIRED_SLICE_REPORTS,
  W5_N24_E_SLICE_ID,
  W5_N24_E_TECHNICAL_DEBT_DELTA,
  W5_N24_E_TRANSITION_MATRIX,
} from './w5-n24-e-package-close-evidence';
...
describe('W5-N24-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N24_E_SLICE_ID).toBe('W5-N24-e');
    expect(W5_N24_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N24_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N24-a',
      'W5-N24-b',
      'W5-N24-c',
      'W5-N24-d',
    ...
    expect(
      W5_N24_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('implementation chain complete', () => {
    ...
    expect(W5_N24_E_BINDING_FINDINGS.implementationChainComplete).toBe(true);
  });
  ...
  it('dependency chain intact — prior packages closed and consumed, not reopened', () => {
    ...
    expect(W5_N24_E_DEPENDENCY_CHAIN.map((link) => link.packageId)).toEqual([
      'W5-N01',
      'W5-N02',
      'W5-N03',
      'W5-N04',
    ...
    expect(W5_N24_E_BINDING_FINDINGS.dependencyChainIntact).toBe(true);
  });

  it('scheduling foundation chain integrity holds', () => {
    const foundation = verifySchedulingFoundationChain();
    ...
    expect(W5_N24_E_BINDING_FINDINGS.schedulingFoundationChainIntact).toBe(true);
  });
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N24_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });

  it('Honest Product enforcement intact', () => {
    const honest = verifyHonestProduct();
    expect(honest.ok).toBe(true);
    expect(honest.operationalContinuityNotRuntimeScheduling).toBe(true);
    expect(honest.restartRecoveryNotProductionReady).toBe(true);
    expect(honest.inventoryHonestBaselineIntact).toBe(true);
    expect(honest.schedulingFunctionalNotAuthorized).toBe(true);
    expect(honest.schedulingDoesNotCalculateEligibilityOrExecute).toBe(true);
    expect(honest.runtimeSchedulingNotClaimed).toBe(true);
    expect(W5_N24_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N24_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N24_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, scheduling functional, or Wave 5 COMPLETE', () => {
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.w5N24CompleteClaimed).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.schedulingFunctionalClaimed).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.runtimeSchedulingImplemented).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.backoffCalculationImplemented).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.eligibilityDeterminationImplemented).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.executionImplemented).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.retryEngineIntroduced).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N24_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
    expect(diagnostics.schedulingFoundation.ok).toBe(true);
  });
});

describe('W5-N24-e package close evidence — integration / planning', () => {
  it('transition safety answers hold', () => {
    ...
    expect(answers.w5N24CompleteNotClaimed).toBe(true);
    expect(answers.notificationPlatformCompleteNotClaimed).toBe(true);
    expect(answers.schedulingFunctionalNotClaimed).toBe(true);
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newRecoveryLogic).toBe(false);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.newOperationalContinuityLogic).toBe(false);
  });

  it('required Close Evidence and slice reports exist', () => {
    for (const name of W5_N24_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N24_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n24-a-inventory.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n24-overview.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n24-validation-plan.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n24-planning-summary.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'wave-5-progress.md'))).toBe(true);
  });

  it('status docs: W5-N24 CLOSED by Product Owner; Wave 5 COMPLETE not claimed', () => {
    const progress = readWave5('wave-5-progress.md');
    const overview = readWave5('w5-n24-overview.md');
    const close = readWave5('w5-n24-close-package-report.md');
    const summary = readWave5('w5-n24-package-summary.md');
    const finalIntegration = readWave5('w5-n24-final-integration-verification.md');
    const poClose = readWave5('w5-n24-product-owner-close-record.md');
    expect(existsSync(join(WAVE5, 'w5-n24-final-integration-verification.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n24-product-owner-close-record.md'))).toBe(true);
    expect(progress).toMatch(/W5-N24-a|W5-N24-b|W5-N24-c|W5-N24-d|W5-N24-e/);
    expect(progress).toMatch(/CLOSED by Product Owner|W5-N24\s+\*\*CLOSED\*\*/i);
    ...
    expect(overview).toMatch(/CLOSED by Product Owner|W5-N24[\s\S]*CLOSED/i);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /runtime scheduling[\s\S]*not|Do NOT.*runtime scheduling|Scheduling Foundation only|Do not declare.*runtime scheduling/i,
    );
  ...
  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N24_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N24_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N24_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N24_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N24_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N24_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(
      /without runtime scheduling/i,
    );
    expect(W5_N24_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N24_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W5_N24_E_TECHNICAL_DEBT_DELTA.deferred).toContain(
      'Final Package Integration Verification',
    );
  });

  it('platform readiness exposes notification platform retry scheduling continuity without implementation controls', () => {
    ...
    expect(view).toMatch(/notificationPlatformRetryScheduling/);
    expect(view).toMatch(/Notification Platform Retry Scheduling/);
    expect(view).not.toMatch(/runtime-scheduling-control|Scheduler Engine UI|Retry Engine UI/i);
    expect(api).toMatch(/notificationPlatformRetryScheduling/);
    expect(api).toMatch(/NotificationPlatformRetrySchedulingContinuityView/);
    expect(readiness).toMatch(/notificationPlatformRetryScheduling/);
    expect(readiness).toMatch(/NotificationPlatformRetrySchedulingContinuityView/);
  });

  it('mandatory questions expected answers hold', () => {
    expect(verifyOperationalChain().ok).toBe(true);
    expect(W5_N24_E_APPROVED_SLICES).toHaveLength(4);
    expect(W5_N24_E_ARCHITECTURE_CLAIMS.schedulingFunctionalClaimed).toBe(false);
