...
import {
  W5_N07_A_CONFORMANCE_SLICE_ID,
  W5_N07_A_REQUIRED_REPORTS,
  buildNotificationPlatformDispatchDiagnostics,
...
} from './w5-n07-a-notification-platform-dispatch';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N07-a notification platform dispatch conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N07_A_CONFORMANCE_SLICE_ID).toBe('W5-N07-a');
    expect(W5_N07_A_REQUIRED_REPORTS.length).toBe(6);
  });

  it('inventory completeness verifies required ownership rows and no platform dispatch authorization', () => {
    const inventory = verifyInventoryCompleteness();
    expect(inventory.ok).toBe(true);
    expect(inventory.rowCount).toBeGreaterThanOrEqual(45);
    expect(inventory.noRowAuthorizesPlatformDispatchFunctional).toBe(true);
    expect(inventory.noRowAuthorizesW5N07Complete).toBe(true);
  ...
  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    ...
    expect(honest.platformDispatchNotAuthorized).toBe(true);
    expect(honest.dispatchOnlyNotControlPlane).toBe(true);
  });
  ...
  it('architecture integrity: ownership preserved; no duplicate subsystem; Exchange Adapter untouched', () => {
    ...
    expect(W5_N07_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('honesty boundaries: dispatch foundation, platform ready, and per-channel rules frozen', () => {
    const honesty = verifyHonestyBoundaries();
    expect(honesty.ok).toBe(true);
    expect(honesty.dispatchFoundationNotLiveTrading).toBe(true);
    expect(honesty.platformReadyRequiresEvidence).toBe(true);
    expect(honesty.deliveryNotDispatchComplete).toBe(true);
  });

  it('binding findings: Notification Platform Dispatch cannot function after slice a', () => {
    ...
    expect(W5_N07_A_ARCHITECTURE_CLAIMS.platformDispatchFunctionalClaimed).toBe(false);
    expect(W5_N07_A_ARCHITECTURE_CLAIMS.w5N07CompleteClaimed).toBe(false);
    expect(W5_N07_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N07_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
  ...
  it('diagnostics roll-up passes for slice a scope', () => {
    const diagnostics = buildNotificationPlatformDispatchDiagnostics();
...
describe('W5-N07-a notification platform dispatch conformance — integration / planning', () => {
  it('required reports exist for W5-N07-a', () => {
    for (const name of W5_N07_A_REQUIRED_REPORTS) {
