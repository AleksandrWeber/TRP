...
describe('W5-N10-e package close evidence — integration / planning', () => {
  ...
  it('required close evidence reports exist', () => {
    ...
    expect(existsSync(join(WAVE5, 'w5-n10-product-owner-close-record.md'))).toBe(true);
  });
  ...
  it('status docs: W5-N10 CLOSED by Product Owner; Wave 5 COMPLETE not claimed', () => {
    ...
    const poClose = readWave5('w5-n10-product-owner-close-record.md');
    ...
    expect(progress).toMatch(/CLOSED by Product Owner|W5-N10[\s\S]*CLOSED/i);
    ...
    expect(overview).toMatch(/CLOSED by Product Owner|W5-N10[\s\S]*CLOSED/i);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /Notification Platform Worker Execution implemented[\s\S]*not|not[\s\S]*Notification Platform Worker Execution implemented/i,
    );
