...
import {
  W5_N16_A_CONFORMANCE_SLICE_ID,
  W5_N16_A_REQUIRED_REPORTS,
  buildNotificationPlatformMetricsDiagnostics,
...
} from './w5-n16-a-notification-platform-metrics';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N16-a notification platform metrics conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N16_A_CONFORMANCE_SLICE_ID).toBe('W5-N16-a');
    expect(W5_N16_A_REQUIRED_REPORTS.length).toBe(6);
  });

  it('inventory completeness verifies required ownership rows and no platform metrics authorization', () => {
    const inventory = verifyInventoryCompleteness();
    expect(inventory.ok).toBe(true);
    expect(inventory.rowCount).toBeGreaterThanOrEqual(45);
    expect(inventory.noRowAuthorizesPlatformMetricsFunctional).toBe(true);
    expect(inventory.noRowAuthorizesW5N16Complete).toBe(true);
  ...
  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    ...
    expect(honest.platformMetricsNotAuthorized).toBe(true);
  ...
  it('architecture integrity: ownership preserved; no duplicate subsystem; Exchange Adapter untouched', () => {
    ...
    expect(W5_N16_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('honesty boundaries: workers foundation, platform ready, and per-channel rules frozen', () => {
    ...
    expect(honesty.metricsFoundationNotLiveTrading).toBe(true);
    expect(honesty.platformReadyRequiresMetricsEvidence).toBe(true);
    expect(honesty.telemetryFoundationNotMetricsComplete).toBe(true);
  });

  it('binding findings: Notification Platform Metrics cannot function after slice a', () => {
    ...
    expect(W5_N16_A_ARCHITECTURE_CLAIMS.platformMetricsFunctionalClaimed).toBe(false);
    expect(W5_N16_A_ARCHITECTURE_CLAIMS.w5N16CompleteClaimed).toBe(false);
    expect(W5_N16_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N16_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
  ...
  it('diagnostics roll-up passes for slice a scope', () => {
    const diagnostics = buildNotificationPlatformMetricsDiagnostics();
...
describe('W5-N16-a notification platform metrics conformance — integration / planning', () => {
  it('required reports exist for W5-N16-a', () => {
    for (const name of W5_N16_A_REQUIRED_REPORTS) {
