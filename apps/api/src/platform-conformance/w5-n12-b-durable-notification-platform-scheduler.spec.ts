...
import {
  ...
  W5_N12_B_CANONICAL_ANCHOR_FIELDS,
  W5_N12_B_DURABLE_COVERAGE,
  W5_N12_B_EXPLICIT_OUT,
  W5_N12_B_NEW_PERSISTED_ARTIFACT_IDS,
  W5_N12_B_NOTIFICATION_OWNER,
  W5_N12_B_PREEXISTING_SURVIVE_ARTIFACT_IDS,
  W5_N12_B_SLICE_ID,
  W5_N12_B_TECHNICAL_DEBT_DELTA,
  W5_N12_B_TRANSITION_MATRIX,
  newPersistedInventoryRows,
  persistedArtifactIds,
  preexistingSurviveInventoryRows,
  verifyInventorySynchronization,
} from './w5-n12-b-durable-notification-platform-scheduler';
  ...
  return {
    workspaceNotificationPlatformSchedulerAnchor: {
      upsert: async ({
        ...
        const key = `${workspaceId}:${schedulerAnchorId}`;
      ...
      }) => rows.get(`${workspaceId}:${schedulerAnchorId}`) ?? null,
    },
    _rows: rows,
  };
}

describe('W5-N12-b durable notification platform scheduler — unit', () => {
  it('persistence correctness: anchor upserts workspace scheduler row', async () => {
    ...
    const loaded = await service.loadAnchor('ws-a', 'scheduler-1');
    expect(loaded).toMatchObject({
      workspaceId: 'ws-a',
      schedulerAnchorId: 'scheduler-1',
      schedulerState: 'anchor-recorded',
    });
    expect(loaded).not.toHaveProperty('schedulerRuntimeState');
  ...
  it('artifact coverage: only approved new SURVIVE row is persisted by this slice', () => {
    expect([...persistedArtifactIds()].sort()).toEqual(
      [...W5_N12_B_NEW_PERSISTED_ARTIFACT_IDS].sort(),
    );
    const rows = newPersistedInventoryRows();
    expect(rows.length).toBe(W5_N12_B_NEW_PERSISTED_ARTIFACT_IDS.length);
    for (const row of rows) {
      expect(row.artifactId).toBe('persist-notification-platform-scheduler-anchor');
  ...
  it('pre-existing SURVIVE rows remain on notification-delivery owner', () => {
    ...
    expect(preexisting.length).toBe(W5_N12_B_PREEXISTING_SURVIVE_ARTIFACT_IDS.length);
    expect(
      preexisting.every(
        (row) =>
  ...
  it('ownership: platform scheduler persistence remains on notification-delivery owner only', () => {
    expect(W5_N12_B_NOTIFICATION_OWNER).toBe('notification-delivery');
    for (const row of W5_N12_B_DURABLE_COVERAGE) {
      expect(row.owner).toBe('notification-delivery');
    }
  });

  it('canonical anchor fields are frozen for W5-N12-b', () => {
    expect(W5_N12_B_CANONICAL_ANCHOR_FIELDS).toEqual(
...
describe('W5-N12-b durable notification platform scheduler — integration', () => {
  it('persistence lifecycle: no recovery / scheduler runtime / functional claims from this slice', () => {
    expect(W5_N12_B_SLICE_ID).toBe('W5-N12-b');
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.automaticRestartRecovery).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.operationalContinuityGuaranteed).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.platformSchedulerImplementation).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.productionTransportIo).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.platformSchedulerFunctional).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.platformSchedulerRestartSurvivalClaimed).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.schedulerRuntimeImplemented).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.schedulingEngineImplemented).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.schedulerExecutionImplemented).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.retryOrchestrationImplemented).toBe(false);
    expect(W5_N12_B_ARCHITECTURE_CLAIMS.deadLetterProcessingImplemented).toBe(false);
  ...
  it('explicit OUT covers restart recovery (W5-N12-b scope)', () => {
    expect(W5_N12_B_EXPLICIT_OUT).toEqual(
      expect.arrayContaining([
        'restart-recovery',
  ...
  it('schema and module wiring evidence exist', () => {
    expect(
      existsSync(
        join(
          REPO_ROOT,
          'apps/api/src/modules/notification-delivery/domain/durable-notification-platform-scheduler-anchor.ts',
        ),
      ),
    ).toBe(true);
    expect(
  ...
  it('required reports exist for W5-N12-b', () => {
