...
import { W5_N09_A_ARCHITECTURE_CLAIMS } from './w5-n09-a-notification-platform-workers-inventory';
...
import {
  ...
  verifyWorkersFoundationChain,
  W5_N09_E_APPROVED_SLICES,
  W5_N09_E_ARCHITECTURE_CLAIMS,
  W5_N09_E_BINDING_FINDINGS,
  W5_N09_E_CAPABILITY_EVOLUTION,
  W5_N09_E_DEPENDENCY_CHAIN,
  W5_N09_E_INTEGRITY_NON_EXPANSION,
  W5_N09_E_NOTIFICATION_OWNER,
  W5_N09_E_OPERATIONAL_MATURITY,
  W5_N09_E_REQUIRED_REPORTS,
  W5_N09_E_REQUIRED_SLICE_REPORTS,
  W5_N09_E_SLICE_ID,
  W5_N09_E_TECHNICAL_DEBT_DELTA,
  W5_N09_E_TRANSITION_MATRIX,
} from './w5-n09-e-package-close-evidence';
...
describe('W5-N09-e package close evidence — unit', () => {
  it('approved slices a–d recorded PASS for validation / architecture / security / product', () => {
    expect(W5_N09_E_SLICE_ID).toBe('W5-N09-e');
    expect(W5_N09_E_NOTIFICATION_OWNER).toBe('notification-delivery');
    expect(W5_N09_E_APPROVED_SLICES.map((s) => s.id)).toEqual([
      'W5-N09-a',
      'W5-N09-b',
      'W5-N09-c',
      'W5-N09-d',
    ...
    expect(
      W5_N09_E_APPROVED_SLICES.every(
        (s) =>
          s.validation === 'PASS' &&
          s.architecture === 'PASS' &&
          s.security === 'PASS' &&
  ...
  it('implementation chain complete', () => {
    ...
    expect(W5_N09_E_BINDING_FINDINGS.implementationChainComplete).toBe(true);
  });
  ...
  it('dependency chain intact — prior packages closed and consumed, not reopened', () => {
    ...
    expect(W5_N09_E_DEPENDENCY_CHAIN.map((link) => link.packageId)).toEqual([
      'W5-N01',
      'W5-N02',
      'W5-N03',
      'W5-N04',
    ...
    expect(W5_N09_E_BINDING_FINDINGS.dependencyChainIntact).toBe(true);
  });

  it('workers foundation chain integrity holds', () => {
    const foundation = verifyWorkersFoundationChain();
    ...
    expect(W5_N09_E_BINDING_FINDINGS.workersFoundationChainIntact).toBe(true);
  });
  ...
  it('complete operational journey works', () => {
    ...
    expect(W5_N09_E_BINDING_FINDINGS.operationalJourneyWorks).toBe(true);
  });

  it('Honest Product enforcement intact', () => {
    const honest = verifyHonestProduct();
    expect(honest.ok).toBe(true);
    expect(honest.operationalContinuityNotPlatformWorkersExecution).toBe(true);
    expect(honest.restartRecoveryNotProductionReady).toBe(true);
    expect(honest.inventoryHonestBaselineIntact).toBe(true);
    expect(honest.platformWorkersFunctionalNotAuthorized).toBe(true);
    expect(honest.workerRuntimeNotClaimed).toBe(true);
    expect(W5_N09_E_BINDING_FINDINGS.honestProductEnforcementIntact).toBe(true);
  });
  ...
  it('ownership and architecture integrity hold across a–e', () => {
    ...
    expect(W5_N09_E_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N09_E_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('this slice does not declare package CLOSED, platform workers functional, or Wave 5 COMPLETE', () => {
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.w5N09CompleteClaimed).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.platformWorkersFunctionalClaimed).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.wave5DeclaredComplete).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.finalPackageIntegrationVerificationPerformed).toBe(false);
    expect(W5_N09_E_BINDING_FINDINGS.packageDeclaredClosed).toBe(false);
    const diagnostics = buildCloseEvidenceDiagnostics();
    expect(diagnostics.packageDeclaredClosed).toBe(false);
  });
});

describe('W5-N09-e package close evidence — integration / planning', () => {
  it('transition safety answers hold', () => {
    ...
    expect(answers.w5N09CompleteNotClaimed).toBe(true);
    expect(answers.notificationPlatformCompleteNotClaimed).toBe(true);
    expect(answers.platformWorkersFunctionalNotClaimed).toBe(true);
  ...
  it('architecture non-claims: no new capability / no silent expansion', () => {
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.newCustomerFunctionality).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.newApi).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.newUi).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.newPersistence).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.platformWorkersExecution).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.workerRuntimeImplemented).toBe(false);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.liveTrading).toBe(false);
    expect(W5_N09_E_INTEGRITY_NON_EXPANSION).toContain('Worker Runtime Implementation');
    expect(W5_N09_E_INTEGRITY_NON_EXPANSION).toContain(
      'Final Package Integration Verification Performed',
    );
  });

  it('product verification claims hold across c/d', () => {
    expect(W5_N09_C_ARCHITECTURE_CLAIMS.recoveryDeterministic).toBe(true);
    expect(W5_N09_C_ARCHITECTURE_CLAIMS.recoveryIdempotent).toBe(true);
    expect(
      W5_N09_C_ARCHITECTURE_CLAIMS.notificationPlatformWorkersAnchorStateRestoredAfterRestart,
    ).toBe(true);
    expect(W5_N09_D_ARCHITECTURE_CLAIMS.operationalContinuityDerived).toBe(true);
    expect(W5_N09_D_ARCHITECTURE_CLAIMS.neverHardcodesReady).toBe(true);
    expect(W5_N09_D_ARCHITECTURE_CLAIMS.canFabricateReadiness).toBe(false);
    expect(OPERATIONAL_STATES).toEqual(['Recovering', 'Ready', 'Degraded', 'Unavailable']);
  });

  it('required close evidence reports exist', () => {
    for (const name of W5_N09_E_REQUIRED_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    for (const name of W5_N09_E_REQUIRED_SLICE_REPORTS) {
      expect(existsSync(join(WAVE5, name)), name).toBe(true);
    }
    expect(existsSync(join(WAVE5, 'w5-n09-a-notification-platform-workers-inventory.md'))).toBe(
      true,
    );
    ...
    expect(existsSync(join(WAVE5, 'w5-n09-planning-approval.md'))).toBe(true);
    expect(existsSync(join(WAVE5, 'w5-n09-final-integration-verification.md'))).toBe(false);
    expect(existsSync(join(WAVE5, 'w5-n09-product-owner-close-record.md'))).toBe(false);
  });
  ...
  it('transition / maturity / capability / debt registries are complete', () => {
    expect(W5_N09_E_TRANSITION_MATRIX.before.length).toBeGreaterThan(0);
    expect(W5_N09_E_TRANSITION_MATRIX.after.length).toBeGreaterThan(0);
    expect(W5_N09_E_TRANSITION_MATRIX.stillMissing).toContain(
      'Final Package Integration Verification',
    );
    expect(W5_N09_E_OPERATIONAL_MATURITY.after).toContain('Package Close Evidence');
    expect(W5_N09_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(/restart recovery/);
    expect(W5_N09_E_CAPABILITY_EVOLUTION.packageClosedCapability).toMatch(
      /without platform workers execution/,
    );
    expect(W5_N09_E_TECHNICAL_DEBT_DELTA.resolved.length).toBeGreaterThan(0);
    expect(W5_N09_E_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
  });

  it('platform readiness exposes notification platform workers continuity without implementation controls', () => {
    ...
    expect(api).toContain('NotificationPlatformWorkersContinuityView');
    expect(view).toMatch(/Notification Platform Workers|notificationPlatformWorkers/i);
  ...
  it('status docs: W5-N09-e COMPLETE awaiting PO Review; package not CLOSED; Wave 5 COMPLETE not claimed', () => {
    const progress = readWave5('wave-5-progress.md');
    const overview = readWave5('wave-5-overview.md');
    const close = readWave5('w5-n09-package-close-report.md');
    const summary = readWave5('w5-n09-package-summary.md');
    expect(progress).toMatch(/W5-N09-a|W5-N09-b|W5-N09-c|W5-N09-d|W5-N09-e/);
    expect(progress).toMatch(/W5-N09-e[\s\S]*COMPLETE|COMPLETE[\s\S]*W5-N09-e/i);
    expect(progress).toMatch(/Awaiting Product Owner Review|Awaiting PO Review/i);
    expect(progress).toMatch(/Wave 5 COMPLETE[\s\S]*Not claimed|Not claimed[\s\S]*Wave 5 COMPLETE/);
    expect(overview).toMatch(/W5-N09-e[\s\S]*COMPLETE|COMPLETE[\s\S]*W5-N09-e/i);
    expect(overview).toMatch(/STOP/);
    expect(overview).toMatch(
      /Notification Platform Workers implemented[\s\S]*not|not[\s\S]*Notification Platform Workers implemented/i,
    );
    expect(close).toMatch(/Awaiting Product Owner Review|not CLOSED|Not granted/i);
    expect(summary).toMatch(/Awaiting Product Owner Review|not CLOSED|foundation scope only/i);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.packageCloseEvidenceAssembled).toBe(true);
    expect(W5_N09_E_ARCHITECTURE_CLAIMS.packageDeclaredClosed).toBe(false);
  });
  ...
  it('slice validation reports a–d exist and record PASS', () => {
    for (const slice of ['a', 'b', 'c', 'd'] as const) {
      const report = readWave5(`w5-n09-${slice}-validation-report.md`);
