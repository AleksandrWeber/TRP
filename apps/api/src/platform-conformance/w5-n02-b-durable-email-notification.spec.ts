...
import {
  ...
  W5_N02_B_CANONICAL_ANCHOR_FIELDS,
  W5_N02_B_DURABLE_COVERAGE,
  W5_N02_B_EXPLICIT_OUT,
  W5_N02_B_NEW_PERSISTED_ARTIFACT_IDS,
  W5_N02_B_NOTIFICATION_OWNER,
  W5_N02_B_PREEXISTING_SURVIVE_ARTIFACT_IDS,
  W5_N02_B_SLICE_ID,
  W5_N02_B_TECHNICAL_DEBT_DELTA,
  W5_N02_B_TRANSITION_MATRIX,
  newPersistedInventoryRows,
  persistedArtifactIds,
  preexistingSurviveInventoryRows,
} from './w5-n02-b-durable-email-notification';
...
describe('W5-N02-b durable email notification — unit', () => {
  ...
  it('artifact coverage: only approved new SURVIVE row is persisted by this slice', () => {
    expect([...persistedArtifactIds()].sort()).toEqual(
      [...W5_N02_B_NEW_PERSISTED_ARTIFACT_IDS].sort(),
    );
    const rows = newPersistedInventoryRows();
    expect(rows.length).toBe(W5_N02_B_NEW_PERSISTED_ARTIFACT_IDS.length);
    for (const row of rows) {
      expect(row.artifactId).toBe('persist-email-notification-anchor');
  ...
  it('pre-existing SURVIVE rows remain on notification-delivery / secret-vault owners', () => {
    ...
    expect(preexisting.length).toBe(W5_N02_B_PREEXISTING_SURVIVE_ARTIFACT_IDS.length);
  ...
  it('ownership: email notification persistence remains on notification-delivery owner only', () => {
    expect(W5_N02_B_NOTIFICATION_OWNER).toBe('notification-delivery');
    for (const row of W5_N02_B_DURABLE_COVERAGE) {
      expect(row.owner).toBe('notification-delivery');
    }
  });

  it('canonical anchor fields are frozen for W5-N02-b', () => {
    expect(W5_N02_B_CANONICAL_ANCHOR_FIELDS).toEqual(
  ...
  it('transition matrix: inventory → durable persistence → restart recovery still missing', () => {
    expect(W5_N02_B_TRANSITION_MATRIX.before).toContain('Inventory');
    expect(W5_N02_B_TRANSITION_MATRIX.after).toContain('Durable Persistence');
    expect(W5_N02_B_TRANSITION_MATRIX.stillMissing.some((item) => item.includes('Restart'))).toBe(
      true,
    );
  });
});

describe('W5-N02-b durable email notification — integration', () => {
  it('persistence lifecycle: no recovery / SMTP / outbound delivery claims from this slice', () => {
    expect(W5_N02_B_SLICE_ID).toBe('W5-N02-b');
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.automaticRestartRecovery).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.operationalContinuityGuaranteed).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.smtpImplementation).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.outboundNotificationDelivery).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.emailNotificationRestartSurvivalClaimed).toBe(false);
    expect(W5_N02_B_ARCHITECTURE_CLAIMS.exchangeAdapterUntouched).toBe(true);
  });

  it('technical debt delta: durable foundation resolved; recovery deferred', () => {
    expect(W5_N02_B_TECHNICAL_DEBT_DELTA.resolved).toContain(
      'Durable Email Notification Foundation',
    );
    expect(W5_N02_B_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
    expect(W5_N02_B_TECHNICAL_DEBT_DELTA.deferred.some((item) => item.includes('restart'))).toBe(
      true,
    );
  });

  it('explicit OUT covers restart recovery and W5-N02-c', () => {
    expect(W5_N02_B_EXPLICIT_OUT).toEqual(
      expect.arrayContaining(['restart-recovery', 'w5-n02-c', 'smtp-transport']),
    );
  });
  ...
  it('required reports exist for W5-N02-b', () => {
