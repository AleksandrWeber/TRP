...
import {
  W5_N27_A_ALLOWED_OWNERS,
  W5_N27_A_ARCHITECTURE_CLAIMS,
  W5_N27_A_ARTIFACT_KINDS,
  W5_N27_A_BINDING_FINDINGS,
  W5_N27_A_DECISION_CLASSIFICATIONS,
  W5_N27_A_CAPABILITY_CATEGORIES,
  W5_N27_A_EXPLICIT_OUT,
  W5_N27_A_HONEST_PRODUCT_BASELINE,
  W5_N27_A_REQUIRED_ARTIFACT_KINDS,
  W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY,
  W5_N27_A_SLICE_ID,
  W5_N27_A_SUBSTRATE_OWNERS,
  W5_N27_A_TECHNICAL_DEBT_DELTA,
  artifactIds,
...
describe('W5-N27-a notification retry scheduling decision projection inventory — unit', () => {
  it('inventory completeness: every required artifact kind appears', () => {
    const kinds = new Set(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.map((row) => row.kind),
    );
    for (const kind of W5_N27_A_REQUIRED_ARTIFACT_KINDS) {
      expect(kinds.has(kind)).toBe(true);
    }
    expect(W5_N27_A_ARTIFACT_KINDS).toEqual([...W5_N27_A_REQUIRED_ARTIFACT_KINDS]);
  });
  ...
  it('inventory completeness: every row has required classification and responsibility fields', () => {
    for (const row of W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY) {
      expect(W5_N27_A_ALLOWED_OWNERS).toContain(row.owner);
      expect(W5_N27_A_CAPABILITY_CATEGORIES).toContain(row.capabilityCategory);
      expect(row.purpose.length).toBeGreaterThan(0);
      expect(row.projectionRole.length).toBeGreaterThan(0);
  ...
  it('ownership consistency: core ownership rows stay on notification substrate owners', () => {
    ...
    for (const row of ownership.filter((entry) => coreIds.includes(entry.artifactId))) {
      expect(W5_N27_A_SUBSTRATE_OWNERS).toContain(row.owner);
    }
    expect(
  ...
  it('platform coverage: N17–N22 consume, missing eligibility gaps, PC-06 routing', () => {
    ...
    expect(ids.has('missing-unified-platform-decision-projection-view')).toBe(true);
    expect(ids.has('missing-projection-persistence')).toBe(true);
    expect(ids.has('missing-projection-recovery')).toBe(true);
    expect(ids.has('missing-projection-operational-continuity')).toBe(true);
    expect(ids.has('runtime-pc06-resolve-delivery-routing')).toBe(true);
    expect(ids.has('config-projection-rule-representation')).toBe(true);
    expect(ids.has('projection-result-model-representation')).toBe(true);
  });
  ...
  it('honesty: no row authorizes eligibility functional; slice a open with missing foundations', () => {
    expect(W5_N27_A_BINDING_FINDINGS.projectionFunctionalAuthorized).toBe(false);
    expect(W5_N27_A_BINDING_FINDINGS.projectionFunctionsAfterSliceA).toBe(false);
    expect(W5_N27_A_BINDING_FINDINGS.customerVisibleFeatureFromSliceA).toBe(false);
    expect(W5_N27_A_BINDING_FINDINGS.w5N17DeliveryReliabilityExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N18RetryExecutionExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N19RetrySchedulingExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N20RetryPolicyExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N21RetryBackoffExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N22RetryBackoffCalculationExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N23RetryEligibilityExists).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.w5N24RetrySchedulingExists).toBe(true);
    ...
    expect(W5_N27_A_BINDING_FINDINGS.projectionRecoveryMissing).toBe(true);
    ...
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotPerformBackoffCalculation).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotScheduleRetries).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotExecuteRetries).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotOwnRetryLifecycle).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotOwnTimers).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotOwnWorkers).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryDoesNotOwnOrchestration).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.inventoryOutputInformationalOnly).toBe(true);
  });

  it('capability categories cover implemented, infrastructure, planned, not-implemented, future-roadmap', () => {
    for (const category of W5_N27_A_CAPABILITY_CATEGORIES) {
  ...
  it('explicit OUT surfaces cover eligibility evaluation, b–e, engines, platforms, Live Trading', () => {
    ...
    expect(ids.has('out-w5-n27-b')).toBe(true);
    expect(ids.has('out-w5-n27-c')).toBe(true);
    expect(ids.has('out-w5-n27-d')).toBe(true);
    expect(ids.has('out-w5-n27-e')).toBe(true);
    expect(ids.has('out-runtime-scheduler')).toBe(true);
  ...
  it('W5-N17 through W5-N22 foundations consumed not reopened', () => {
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n17-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n18-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n19-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n20-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n21-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n22-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n24-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n25-reference');
    expect(W5_N27_A_ALLOWED_OWNERS).toContain('w5-n26-reference');
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N17Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N18Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N19Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N20Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N21Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N22Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N24Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N25Reopened).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.w5N26Reopened).toBe(false);
  });

  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    expect(W5_N27_A_HONEST_PRODUCT_BASELINE.implementedCapabilities[0]).toContain('None');
    expect(
      W5_N27_A_HONEST_PRODUCT_BASELINE.infrastructureCapabilities.length,
    ).toBeGreaterThanOrEqual(8);
    expect(W5_N27_A_HONEST_PRODUCT_BASELINE.plannedCapabilities.length).toBeGreaterThanOrEqual(1);
    expect(
      W5_N27_A_HONEST_PRODUCT_BASELINE.notYetImplementedCapabilities.length,
    ).toBeGreaterThanOrEqual(5);
    expect(
      W5_N27_A_HONEST_PRODUCT_BASELINE.futureRoadmapCapabilities.length,
    ).toBeGreaterThanOrEqual(2);
  });

  it('technical debt delta: inventory and persistence baselines resolved; recovery / continuity / Close deferred; nothing introduced', () => {
  ...
  it('persistence gap resolved by W5-N27-b; recovery and continuity remain open; unified view remains open', () => {
    expect(
    ...
    expect(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.find(
        (entry) => entry.artifactId === 'persist-candidate-projection-anchor',
      )?.existsToday,
    ).toBe(true);
    expect(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.find(
    ...
    expect(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.find(
        (entry) => entry.artifactId === 'missing-projection-recovery',
      )?.existsToday,
    ).toBe(false);
    expect(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.find(
    ...
    expect(
      W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.find(
        (entry) => entry.artifactId === 'missing-unified-platform-decision-projection-view',
      )?.classification,
    ).toBe('EPHEMERAL');
    expect(W5_N27_A_BINDING_FINDINGS.projectionPersistenceMissing).toBe(false);
    expect(W5_N27_A_BINDING_FINDINGS.projectionRecoveryMissing).toBe(true);
...
describe('W5-N27-a notification retry scheduling decision projection inventory — integration / planning', () => {
  it('planning consistency: slice id and architecture claims remain inventory-only', () => {
    expect(W5_N27_A_SLICE_ID).toBe('W5-N27-a');
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.newBoundedContext).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.duplicateNotificationSubsystem).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.duplicateDecisionProjectionSubsystem).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.duplicateRetrySubsystem).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.duplicateRoutingEngine).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.runtimeSchedulerIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.retryEngineIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.schedulerPlatformIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.workerIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.timerImplementationIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.runtimeDecisionProjectionIntroduced).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.ownershipBoundariesChanged).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.exchangeAdapterUntouched).toBe(true);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.inventoryOnly).toBe(true);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.notificationControlPlane).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.customerVisibleFeature).toBe(false);
    expect(W5_N27_A_BINDING_FINDINGS.ownershipBoundariesVerified).toBe(true);
    expect(W5_N27_A_BINDING_FINDINGS.architecturalDeviations).toBe(false);
  });

  it('master plan consistency: inventory claims leave Master Plan / V2 / Wave 1–4 unchanged', () => {
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.masterPlanModified).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.version2Redesigned).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.wave1Modified).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.wave2Modified).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.wave3Modified).toBe(false);
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.wave4Modified).toBe(false);
  ...
  it('architecture consistency: no Runtime Scheduler / Retry Engine / new persistence owner', () => {
    expect(W5_N27_A_ARCHITECTURE_CLAIMS.newSourceOfTruth).toBe(false);
    for (const item of [
      'runtime-scheduling',
      'runtime-scheduler',
      'retry-engine',
      ...
      expect(W5_N27_A_EXPLICIT_OUT).toContain(item);
    }
  });

  it('architecture consistency: evidence paths exist on disk for existsToday rows', () => {
    for (const row of W5_N27_A_RETRY_SCHEDULING_DECISION_PROJECTION_INVENTORY.filter(
