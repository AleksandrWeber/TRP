...
import {
  W5_N06_A_CONFORMANCE_SLICE_ID,
  W5_N06_A_REQUIRED_REPORTS,
  buildNotificationPlatformDeliveryDiagnostics,
...
} from './w5-n06-a-notification-platform-delivery';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N06-a notification platform delivery conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N06_A_CONFORMANCE_SLICE_ID).toBe('W5-N06-a');
    expect(W5_N06_A_REQUIRED_REPORTS.length).toBe(6);
  });

  it('inventory completeness verifies required ownership rows and no platform delivery authorization', () => {
    const inventory = verifyInventoryCompleteness();
    expect(inventory.ok).toBe(true);
    expect(inventory.rowCount).toBeGreaterThanOrEqual(45);
    expect(inventory.noRowAuthorizesPlatformDeliveryFunctional).toBe(true);
    expect(inventory.noRowAuthorizesW5N06Complete).toBe(true);
  ...
  it('honest product baseline distinguishes implemented from infrastructure and deferred work', () => {
    ...
    expect(honest.platformDeliveryNotAuthorized).toBe(true);
  ...
  it('architecture integrity: ownership preserved; no duplicate subsystem; Exchange Adapter untouched', () => {
    ...
    expect(W5_N06_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('honesty boundaries: delivery foundation, platform ready, and per-channel rules frozen', () => {
    const honesty = verifyHonestyBoundaries();
    expect(honesty.ok).toBe(true);
    expect(honesty.deliveryFoundationNotLiveTrading).toBe(true);
    expect(honesty.platformReadyRequiresEvidence).toBe(true);
    expect(honesty.perChannelNotPlatformComplete).toBe(true);
  });

  it('binding findings: Notification Platform Delivery cannot function after slice a', () => {
    ...
    expect(W5_N06_A_ARCHITECTURE_CLAIMS.platformDeliveryFunctionalClaimed).toBe(false);
    expect(W5_N06_A_ARCHITECTURE_CLAIMS.w5N06CompleteClaimed).toBe(false);
    expect(W5_N06_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N06_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
  ...
  it('diagnostics roll-up passes for slice a scope', () => {
    const diagnostics = buildNotificationPlatformDeliveryDiagnostics();
...
describe('W5-N06-a notification platform delivery conformance — integration / planning', () => {
  it('required reports exist for W5-N06-a', () => {
    for (const name of W5_N06_A_REQUIRED_REPORTS) {
