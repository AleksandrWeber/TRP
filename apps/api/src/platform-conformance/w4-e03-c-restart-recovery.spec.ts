...
import {
  ...
  W4_E03_C_EXCHANGE_CONNECTIVITY_OWNER,
  W4_E03_C_EXPLICIT_OUT,
  W4_E03_C_RECOVERED_ARTIFACT_IDS,
  W4_E03_C_SLICE_ID,
  W4_E03_C_TECHNICAL_DEBT_DELTA,
  W4_E03_C_TRANSITION_MATRIX,
} from './w4-e03-c-restart-recovery';
...
describe('W4-E03-c OKX exchange connectivity restart recovery — unit', () => {
  it('ownership remains exchange-adapter only', () => {
    expect(W4_E03_C_EXCHANGE_CONNECTIVITY_OWNER).toBe('exchange-adapter');
  });
  ...
  it('missing persisted state recovers empty without fabrication', async () => {
    const repository = new PrismaOkxExchangeConnectivityStateRepository(
...
describe('W4-E03-c OKX exchange connectivity restart recovery — integration', () => {
  it('recover persisted OKX exchange connectivity state after normal restart', async () => {
    ...
    expect(W4_E03_C_ARCHITECTURE_CLAIMS.normalProcessRestartRecovery).toBe(true);
  ...
  it('recovery idempotency: hydrate twice yields same diagnostics', async () => {
    ...
    const service = new OkxExchangeConnectivityRestartRecoveryService(
      new PrismaOkxExchangeConnectivityStateRepository(createPrismaMock([toRow(state)]) as never),
      new OkxExchangeConnectivityRecoveryStore(),
    );
  ...
  it('transition matrix documents persistence → recovery → still missing', () => {
    expect(W4_E03_C_TRANSITION_MATRIX.before).toContain('Durable persistence (W4-E03-b)');
    expect(W4_E03_C_TRANSITION_MATRIX.after).toContain('Restart recovery (W4-E03-c)');
    expect(W4_E03_C_TRANSITION_MATRIX.stillMissing).toContain('Operational Continuity (W4-E03-d)');
  });

  it('technical debt delta resolves W4-E03 restart recovery foundation', () => {
    expect(W4_E03_C_TECHNICAL_DEBT_DELTA.resolved[0]).toMatch(/restart recovery/i);
    expect(W4_E03_C_TECHNICAL_DEBT_DELTA.introduced).toEqual([]);
  });

  it('explicit OUT covers operational continuity and W4-E03-d', () => {
    expect(W4_E03_C_EXPLICIT_OUT).toEqual(
      expect.arrayContaining(['operational-continuity', 'w4-e03-d', 'rest-connectivity']),
    );
  });

  it('architecture claims deny operational continuity and exchange connectivity complete', () => {
    expect(W4_E03_C_SLICE_ID).toBe('W4-E03-c');
    expect(W4_E03_C_ARCHITECTURE_CLAIMS.operationalContinuity).toBe(false);
    expect(W4_E03_C_ARCHITECTURE_CLAIMS.exchangeConnectivityCompleteClaimed).toBe(false);
    expect(W4_E03_C_ARCHITECTURE_CLAIMS.newPersistenceOwner).toBe(false);
    expect(W4_E03_C_RECOVERED_ARTIFACT_IDS.length).toBe(1);
  });

  it('required reports exist for W4-E03-c', () => {
