...
import {
  W5_N10_A_CONFORMANCE_SLICE_ID,
  W5_N10_A_REQUIRED_REPORTS,
  buildNotificationPlatformWorkerExecutionDiagnostics,
...
} from './w5-n10-a-notification-platform-worker-execution';

const REPO_ROOT = join(__dirname, '../../../..');
const WAVE5 = join(REPO_ROOT, 'docs/project/version-3/wave-5');

describe('W5-N10-a notification platform workers conformance — unit', () => {
  it('slice id and required report list', () => {
    expect(W5_N10_A_CONFORMANCE_SLICE_ID).toBe('W5-N10-a');
    expect(W5_N10_A_REQUIRED_REPORTS.length).toBe(6);
  });
  ...
  it('architecture integrity: ownership preserved; no duplicate subsystem; Exchange Adapter untouched', () => {
    ...
    expect(W5_N10_A_BINDING_FINDINGS.ownershipBoundariesChanged).toBe(false);
  ...
  it('binding findings: Notification Platform Workers cannot function after slice a', () => {
    ...
    expect(W5_N10_A_ARCHITECTURE_CLAIMS.platformWorkerExecutionFunctionalClaimed).toBe(false);
    expect(W5_N10_A_ARCHITECTURE_CLAIMS.w5N10CompleteClaimed).toBe(false);
    expect(W5_N10_A_ARCHITECTURE_CLAIMS.notificationPlatformCompleteClaimed).toBe(false);
    expect(W5_N10_A_ARCHITECTURE_CLAIMS.liveTradingClaimed).toBe(false);
  });
  ...
  it('diagnostics roll-up passes for slice a scope', () => {
    const diagnostics = buildNotificationPlatformWorkerExecutionDiagnostics();
...
describe('W5-N10-a notification platform workers conformance — integration / planning', () => {
  it('required reports exist for W5-N10-a', () => {
    for (const name of W5_N10_A_REQUIRED_REPORTS) {
