 ...
 * W5-N11-d adds Notification Platform Worker Runtime operational continuity (derived).
 * W5-N12-d adds Notification Platform Scheduler operational continuity (derived).
 ...
 * W5-N17-d adds Notification Platform Delivery Reliability operational continuity (derived).
...
import { getNotificationPlatformWorkerRuntimeContinuityRecord } from '../notification-delivery/domain/notification-platform-worker-runtime-continuity-status';
import { buildNotificationPlatformWorkerRuntimeContinuityProjection } from '../notification-delivery/domain/notification-platform-worker-runtime-operational-continuity';
import { getNotificationPlatformSchedulerContinuityRecord } from '../notification-delivery/domain/notification-platform-scheduler-continuity-status';
import { buildNotificationPlatformSchedulerContinuityProjection } from '../notification-delivery/domain/notification-platform-scheduler-operational-continuity';
import { getNotificationPlatformRetryContinuityRecord } from '../notification-delivery/domain/notification-platform-retry-continuity-status';
import { buildNotificationPlatformRetryContinuityProjection } from '../notification-delivery/domain/notification-platform-retry-operational-continuity';
import { getNotificationPlatformDeadLetterContinuityRecord } from '../notification-delivery/domain/notification-platform-dead-letter-continuity-status';
...
import { getNotificationPlatformRetryBackoffContinuityRecord } from '../notification-delivery/domain/notification-platform-retry-backoff-continuity-status';
...
import { getNotificationPlatformRetrySchedulingDecisionEvaluationContinuityRecord } from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-evaluation-continuity-status';
...
import { getNotificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionContinuityRecord } from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-projection-publication-consumption-continuity-status';
import { buildNotificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionContinuityProjection } from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-projection-publication-consumption-operational-continuity';
...
export class OperationalContinuityService implements OnApplicationBootstrap {
  ...
  constructor(private readonly audit: OperationalContinuityAudit) {
    ...
    this.projection = buildPlatformOperationalProjection({
      ...
      notificationPlatformWorkerRuntime: buildNotificationPlatformWorkerRuntimeContinuityProjection(
        {
          recovering: true,
          ownerReadiness: 'ready',
          continuity: null,
      ...
      notificationPlatformScheduler: buildNotificationPlatformSchedulerContinuityProjection({
        recovering: true,
        ownerReadiness: 'ready',
        continuity: null,
      }),
      notificationPlatformRetry: buildNotificationPlatformRetryContinuityProjection({
        recovering: true,
        ownerReadiness: 'ready',
        continuity: null,
      }),
      notificationPlatformDeadLetter: buildNotificationPlatformDeadLetterContinuityProjection({
        recovering: true,
        ownerReadiness: 'ready',
        continuity: null,
      }),
      ...
      notificationPlatformReliability: buildNotificationPlatformReliabilityContinuityProjection({
        recovering: true,
        ownerReadiness: 'ready',
        continuity: null,
      }),
      ...
      notificationPlatformRetryBackoff: buildNotificationPlatformRetryBackoffContinuityProjection({
        recovering: true,
        ownerReadiness: 'ready',
        continuity: null,
      }),
  ...
  private buildNotificationPlatformWorkerRuntimeView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformWorkerRuntimeContinuityRecord();
    return buildNotificationPlatformWorkerRuntimeContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformSchedulerView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformSchedulerContinuityRecord();
    return buildNotificationPlatformSchedulerContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetryView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetryContinuityRecord();
    return buildNotificationPlatformRetryContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformDeadLetterView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformDeadLetterContinuityRecord();
    return buildNotificationPlatformDeadLetterContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformTelemetryView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformTelemetryContinuityRecord();
    return buildNotificationPlatformTelemetryContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformReliabilityView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformReliabilityContinuityRecord();
    return buildNotificationPlatformReliabilityContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetrySchedulingView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetrySchedulingContinuityRecord();
    return buildNotificationPlatformRetrySchedulingContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetryBackoffView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetryBackoffContinuityRecord();
    return buildNotificationPlatformRetryBackoffContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetryBackoffCalculationView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetryBackoffCalculationContinuityRecord();
    return buildNotificationPlatformRetryBackoffCalculationContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetrySchedulingDecisionEvaluationView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetrySchedulingDecisionEvaluationContinuityRecord();
    return buildNotificationPlatformRetrySchedulingDecisionEvaluationContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetrySchedulingDecisionProjectionView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity = getNotificationPlatformRetrySchedulingDecisionProjectionContinuityRecord();
    return buildNotificationPlatformRetrySchedulingDecisionProjectionContinuityProjection({
      recovering: input.recovering,
      ownerReadiness: continuity?.ownerReadiness ?? input.ownerReadiness,
      continuity,
    });
  ...
  private buildNotificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionView(input: {
    recovering: boolean;
    ownerReadiness: 'ready' | 'unavailable' | 'degraded';
  }) {
    const continuity =
  ...
  private async finalizeFromBootRegistry(): Promise<PlatformOperationalProjection> {
    ...
    this.projection = buildPlatformOperationalProjection({
      ...
      notificationPlatformWorkerRuntime: this.buildNotificationPlatformWorkerRuntimeView({
        recovering: true,
        ownerReadiness: 'ready',
      }),
      notificationPlatformScheduler: this.buildNotificationPlatformSchedulerView({
        recovering: true,
        ownerReadiness: 'ready',
      }),
      notificationPlatformRetry: this.buildNotificationPlatformRetryView({
        recovering: true,
        ownerReadiness: 'ready',
      }),
      notificationPlatformDeadLetter: this.buildNotificationPlatformDeadLetterView({
        recovering: true,
        ownerReadiness: 'ready',
      }),
      ...
      notificationPlatformRetryBackoff: this.buildNotificationPlatformRetryBackoffView({
        recovering: true,
        ownerReadiness: 'ready',
      }),
      notificationPlatformRetryBackoffCalculation:
        this.buildNotificationPlatformRetryBackoffCalculationView({
          recovering: true,
          ownerReadiness: 'ready',
        }),
    ...
    const notificationPlatformWorkerRuntimeBase = this.buildNotificationPlatformWorkerRuntimeView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformWorkerRuntime = Object.freeze({
      ...notificationPlatformWorkerRuntimeBase,
      recoveryTimestamp:
        notificationPlatformWorkerRuntimeBase.recoveryTimestamp ??
        (notificationPlatformWorkerRuntimeBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformWorkerRuntimeBase.recoveryDurationMs ??
        (notificationPlatformWorkerRuntimeBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformSchedulerBase = this.buildNotificationPlatformSchedulerView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformScheduler = Object.freeze({
      ...notificationPlatformSchedulerBase,
      recoveryTimestamp:
        notificationPlatformSchedulerBase.recoveryTimestamp ??
        (notificationPlatformSchedulerBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformSchedulerBase.recoveryDurationMs ??
        (notificationPlatformSchedulerBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformRetryBase = this.buildNotificationPlatformRetryView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformRetry = Object.freeze({
      ...notificationPlatformRetryBase,
      recoveryTimestamp:
        notificationPlatformRetryBase.recoveryTimestamp ??
        (notificationPlatformRetryBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetryBase.recoveryDurationMs ??
        (notificationPlatformRetryBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformDeadLetterBase = this.buildNotificationPlatformDeadLetterView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformDeadLetter = Object.freeze({
      ...notificationPlatformDeadLetterBase,
      recoveryTimestamp:
        notificationPlatformDeadLetterBase.recoveryTimestamp ??
        (notificationPlatformDeadLetterBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformDeadLetterBase.recoveryDurationMs ??
        (notificationPlatformDeadLetterBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformTelemetryBase = this.buildNotificationPlatformTelemetryView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformTelemetry = Object.freeze({
      ...notificationPlatformTelemetryBase,
      recoveryTimestamp:
        notificationPlatformTelemetryBase.recoveryTimestamp ??
        (notificationPlatformTelemetryBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformTelemetryBase.recoveryDurationMs ??
        (notificationPlatformTelemetryBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    ...
    const notificationPlatformReliabilityBase = this.buildNotificationPlatformReliabilityView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformReliability = Object.freeze({
      ...notificationPlatformReliabilityBase,
      recoveryTimestamp:
        notificationPlatformReliabilityBase.recoveryTimestamp ??
        (notificationPlatformReliabilityBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformReliabilityBase.recoveryDurationMs ??
        (notificationPlatformReliabilityBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    ...
    const notificationPlatformRetrySchedulingBase =
      this.buildNotificationPlatformRetrySchedulingView({
        recovering: false,
        ownerReadiness: 'ready',
      });
    const notificationPlatformRetryScheduling = Object.freeze({
      ...notificationPlatformRetrySchedulingBase,
      recoveryTimestamp:
        notificationPlatformRetrySchedulingBase.recoveryTimestamp ??
        (notificationPlatformRetrySchedulingBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetrySchedulingBase.recoveryDurationMs ??
        (notificationPlatformRetrySchedulingBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    ...
    const notificationPlatformRetryBackoffBase = this.buildNotificationPlatformRetryBackoffView({
      recovering: false,
      ownerReadiness: 'ready',
    });
    const notificationPlatformRetryBackoff = Object.freeze({
      ...notificationPlatformRetryBackoffBase,
      recoveryTimestamp:
        notificationPlatformRetryBackoffBase.recoveryTimestamp ??
        (notificationPlatformRetryBackoffBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetryBackoffBase.recoveryDurationMs ??
        (notificationPlatformRetryBackoffBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformRetryBackoffCalculationBase =
      this.buildNotificationPlatformRetryBackoffCalculationView({
        recovering: false,
        ownerReadiness: 'ready',
      });
    const notificationPlatformRetryBackoffCalculation = Object.freeze({
      ...notificationPlatformRetryBackoffCalculationBase,
      recoveryTimestamp:
        notificationPlatformRetryBackoffCalculationBase.recoveryTimestamp ??
        (notificationPlatformRetryBackoffCalculationBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetryBackoffCalculationBase.recoveryDurationMs ??
        (notificationPlatformRetryBackoffCalculationBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    ...
    const notificationPlatformRetrySchedulingDecisionEvaluationBase =
      this.buildNotificationPlatformRetrySchedulingDecisionEvaluationView({
        recovering: false,
        ownerReadiness: 'ready',
      });
    const notificationPlatformRetrySchedulingDecisionEvaluation = Object.freeze({
      ...notificationPlatformRetrySchedulingDecisionEvaluationBase,
      recoveryTimestamp:
        notificationPlatformRetrySchedulingDecisionEvaluationBase.recoveryTimestamp ??
        (notificationPlatformRetrySchedulingDecisionEvaluationBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetrySchedulingDecisionEvaluationBase.recoveryDurationMs ??
        (notificationPlatformRetrySchedulingDecisionEvaluationBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    const notificationPlatformRetrySchedulingDecisionProjectionBase =
      this.buildNotificationPlatformRetrySchedulingDecisionProjectionView({
        recovering: false,
        ownerReadiness: 'ready',
      });
    const notificationPlatformRetrySchedulingDecisionProjection = Object.freeze({
      ...notificationPlatformRetrySchedulingDecisionProjectionBase,
      recoveryTimestamp:
        notificationPlatformRetrySchedulingDecisionProjectionBase.recoveryTimestamp ??
        (notificationPlatformRetrySchedulingDecisionProjectionBase.operationalState === 'Recovering'
          ? null
          : recoveryTimestamp),
      recoveryDurationMs:
        notificationPlatformRetrySchedulingDecisionProjectionBase.recoveryDurationMs ??
        (notificationPlatformRetrySchedulingDecisionProjectionBase.operationalState === 'Recovering'
          ? null
          : recoveryDurationMs),
    });
    ...
    const notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase =
      this.buildNotificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionView({
        recovering: false,
        ownerReadiness: 'ready',
      });
    const notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumption =
      Object.freeze({
        ...notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase,
        recoveryTimestamp:
          notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase.recoveryTimestamp ??
          (notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase.operationalState ===
          'Recovering'
            ? null
            : recoveryTimestamp),
        recoveryDurationMs:
          notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase.recoveryDurationMs ??
          (notificationPlatformRetrySchedulingDecisionProjectionPublicationConsumptionBase.operationalState ===
          'Recovering'
            ? null
            : recoveryDurationMs),
