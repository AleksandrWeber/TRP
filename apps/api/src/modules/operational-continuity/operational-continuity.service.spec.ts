...
import { buildNotificationPlatformRetryExecutionAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-execution-anchor';
import {
...
} from '../notification-delivery/domain/notification-platform-retry-execution-continuity-status';
import { buildNotificationPlatformRetryExecutionRecoveryDiagnostics } from '../notification-delivery/domain/notification-platform-retry-execution-restart-recovery';
import { buildNotificationPlatformRetrySchedulingAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-scheduling-anchor';
import {
...
import { buildNotificationPlatformRetryBackoffAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-backoff-anchor';
import {
...
import { buildNotificationPlatformRetryEligibilityAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-eligibility-anchor';
import {
  recordNotificationPlatformRetryEligibilityRecoveryStart,
  recordNotificationPlatformRetryEligibilityRecoverySuccess,
  resetNotificationPlatformRetryEligibilityContinuity,
} from '../notification-delivery/domain/notification-platform-retry-eligibility-continuity-status';
import { buildNotificationPlatformRetryEligibilityRecoveryDiagnostics } from '../notification-delivery/domain/notification-platform-retry-eligibility-restart-recovery';
import { buildNotificationPlatformRetrySchedulingDecisionAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-scheduling-decision-anchor';
import {
  ...
  resetNotificationPlatformRetrySchedulingDecisionContinuity,
} from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-continuity-status';
import { buildNotificationPlatformRetrySchedulingDecisionRecoveryDiagnostics } from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-restart-recovery';
import { buildNotificationPlatformRetrySchedulingDecisionEvaluationAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-scheduling-decision-evaluation-anchor';
import {
...
} from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-evaluation-continuity-status';
import { buildNotificationPlatformRetrySchedulingDecisionEvaluationRecoveryDiagnostics } from '../notification-delivery/domain/notification-platform-retry-scheduling-decision-evaluation-restart-recovery';
import { buildNotificationPlatformRetrySchedulingDecisionProjectionAnchorState } from '../notification-delivery/domain/durable-notification-platform-retry-scheduling-decision-projection-anchor';
import {
...
describe('OperationalContinuityService', () => {
  ...
  it('includes notification platform retry execution continuity derived from W5-N18-c recovery record', async () => {
    resetNotificationPlatformRetryExecutionContinuity();
    const anchor = buildNotificationPlatformRetryExecutionAnchorState({
      workspaceId: 'ws-1',
    ...
    recordNotificationPlatformRetryExecutionRecoveryStart();
    recordNotificationPlatformRetryExecutionRecoverySuccess({
      diagnostics: buildNotificationPlatformRetryExecutionRecoveryDiagnostics([anchor.anchor]),
    });
  ...
  it('includes notification platform retry scheduling continuity derived from W5-N19-c recovery record', async () => {
    resetNotificationPlatformRetrySchedulingContinuity();
    const anchor = buildNotificationPlatformRetrySchedulingAnchorState({
      workspaceId: 'ws-1',
      retrySchedulingAnchorId: 'retry-scheduling-1',
      platformRetrySchedulingType: 'eligibility-timing-foundation',
      correlationId: 'corr-1',
      actorId: 'actor-1',
      recordedAt: '2026-09-10T21:30:00.000Z',
      prior: null,
    });
    if (!anchor.ok) throw new Error('expected anchor');
    recordNotificationPlatformRetrySchedulingRecoveryStart();
    recordNotificationPlatformRetrySchedulingRecoverySuccess({
      diagnostics: buildNotificationPlatformRetrySchedulingRecoveryDiagnostics([anchor.anchor]),
    });
  ...
  it('includes notification platform retry backoff continuity derived from W5-N21-c recovery record', async () => {
    resetNotificationPlatformRetryBackoffContinuity();
    const anchor = buildNotificationPlatformRetryBackoffAnchorState({
      workspaceId: 'ws-1',
    ...
    recordNotificationPlatformRetryBackoffRecoveryStart();
    recordNotificationPlatformRetryBackoffRecoverySuccess({
      diagnostics: buildNotificationPlatformRetryBackoffRecoveryDiagnostics([anchor.anchor]),
    });
  ...
  it('includes notification platform retry eligibility continuity derived from W5-N23-c recovery record', async () => {
    resetNotificationPlatformRetryEligibilityContinuity();
    const anchor = buildNotificationPlatformRetryEligibilityAnchorState({
      workspaceId: 'ws-1',
    ...
    if (!anchor.ok) throw new Error('expected eligibility anchor');
    recordNotificationPlatformRetryEligibilityRecoveryStart();
    recordNotificationPlatformRetryEligibilityRecoverySuccess({
      diagnostics: buildNotificationPlatformRetryEligibilityRecoveryDiagnostics([anchor.anchor]),
    });
    ...
    expect(projection.notificationPlatformRetryEligibility?.operationalState).toBe('Ready');
    expect(projection.notificationPlatformRetryEligibility?.canonicalAnchorCount).toBe(1);
    expect(projection.platformState).toBe('Ready');
  });

  it('includes notification platform retry scheduling decision continuity derived from W5-N25-c recovery record', async () => {
    resetNotificationPlatformRetrySchedulingDecisionContinuity();
    const anchor = buildNotificationPlatformRetrySchedulingDecisionAnchorState({
      workspaceId: 'ws-1',
      decisionAnchorId: 'decision-anchor-1',
      platformRetrySchedulingDecisionType: 'decision-description-foundation',
      correlationId: 'corr-1',
      actorId: 'actor-1',
      recordedAt: '2026-09-12T22:40:00.000Z',
      prior: null,
    });
    if (!anchor.ok) throw new Error('expected decision anchor');
    recordNotificationPlatformRetrySchedulingDecisionRecoveryStart();
    recordNotificationPlatformRetrySchedulingDecisionRecoverySuccess({
      diagnostics: buildNotificationPlatformRetrySchedulingDecisionRecoveryDiagnostics([
        anchor.anchor,
      ]),
    });
    ...
    expect(projection.notificationPlatformRetrySchedulingDecision?.operationalState).toBe('Ready');
    expect(projection.notificationPlatformRetrySchedulingDecision?.canonicalAnchorCount).toBe(1);
    expect(projection.platformState).toBe('Ready');
  });

  it('includes notification platform retry scheduling decision evaluation continuity derived from W5-N26-c recovery record', async () => {
    resetNotificationPlatformRetrySchedulingDecisionEvaluationContinuity();
    const anchor = buildNotificationPlatformRetrySchedulingDecisionEvaluationAnchorState({
      workspaceId: 'ws-1',
      ...
      correlationId: 'corr-1',
      actorId: 'actor-1',
      recordedAt: '2026-09-13T22:40:00.000Z',
      prior: null,
    });
    if (!anchor.ok) throw new Error('expected evaluation anchor');
    recordNotificationPlatformRetrySchedulingDecisionEvaluationRecoveryStart();
    recordNotificationPlatformRetrySchedulingDecisionEvaluationRecoverySuccess({
  ...
  it('includes notification platform retry scheduling decision projection continuity derived from W5-N27-c recovery record', async () => {
    resetNotificationPlatformRetrySchedulingDecisionProjectionContinuity();
    const anchor = buildNotificationPlatformRetrySchedulingDecisionProjectionAnchorState({
      workspaceId: 'ws-1',
      ...
      correlationId: 'corr-1',
      actorId: 'actor-1',
      recordedAt: '2026-09-13T23:00:00.000Z',
      prior: null,
    });
    if (!anchor.ok) throw new Error('expected projection anchor');
    recordNotificationPlatformRetrySchedulingDecisionProjectionRecoveryStart();
    recordNotificationPlatformRetrySchedulingDecisionProjectionRecoverySuccess({
