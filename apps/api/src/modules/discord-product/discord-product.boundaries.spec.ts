...
const PRODUCT_ROOT = join(process.cwd(), 'src/modules/discord-product');
...
describe('PC-07 — discord-product ownership boundaries', () => {
  it('may compose Notification Delivery Discord methods; never Orders/Risk/Execution', () => {
      ...
      for (const importPath of importPaths(source)) {
        ...
        for (const forbidden of FORBIDDEN_IMPORT_SEGMENTS) {
          if (normalized.includes(forbidden)) {
            violations.push(`${file.split('/discord-product/')[1]} → ${importPath}`);
          }
        }
      }
    }
  ...
  it('does not let Notification Delivery import the Discord product adapter', () => {
    for (const file of listTsFiles(NOTIFICATION_ROOT)) {
      const source = readFileSync(file, 'utf8');
      expect(source).not.toContain('/discord-product');
    }
  });
});
