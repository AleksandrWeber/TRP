export const KRAKEN_EXCHANGE_CONNECTIVITY_EXCHANGE_IDENTIFIER = 'KRAKEN' as const;

export const KRAKEN_EXCHANGE_CONNECTIVITY_STATE_SCHEMA_VERSION = 1;

export type DurableKrakenExchangeConnectivityState = Readonly<{
  workspaceId: string;
  schemaVersion: number;
  exchangeIdentifier: typeof KRAKEN_EXCHANGE_CONNECTIVITY_EXCHANGE_IDENTIFIER;
...
export type KrakenExchangeConnectivityPersistenceOutcome =
  | Readonly<{ ok: true; state: DurableKrakenExchangeConnectivityState }>
...
function emptyState(
  ...
  return Object.freeze({
    workspaceId,
    schemaVersion: KRAKEN_EXCHANGE_CONNECTIVITY_STATE_SCHEMA_VERSION,
...
/**
 * Build durable Connection Management anchor for Kraken persistence (W4-E04-b).
 * Stores explicit connection id and exchange identifier only — not synthetic Connected.
 */
export function buildKrakenConnectionManagementAnchorState(input: {
  workspaceId: string;
  connectionId: string;
  actorId: string;
  recordedAt: string;
  correlationId?: string | null;
  integrityMetadataHash?: string | null;
  prior: DurableKrakenExchangeConnectivityState | null;
}): KrakenExchangeConnectivityPersistenceOutcome {
...
/**
 * Build durable Exchange Adapter layer anchor for Kraken persistence (W4-E04-b).
 * Stores explicit adapter exchange_connection id only — not runtime connected flag.
 */
export function buildKrakenAdapterLayerAnchorState(input: {
  workspaceId: string;
