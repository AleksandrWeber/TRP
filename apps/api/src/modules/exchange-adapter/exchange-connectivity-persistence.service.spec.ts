...
function createRepository(): ExchangeConnectivityStateRepository & {
  saved: DurableExchangeConnectivityState[];
} {
  const saved: DurableExchangeConnectivityState[] = [];
  let current: DurableExchangeConnectivityState | null = null;
  return {
    saved,
    ...
    loadExchangeConnectivityState: vi.fn(async () => current),
