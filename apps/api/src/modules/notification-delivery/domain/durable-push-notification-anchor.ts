export const PUSH_NOTIFICATION_ANCHOR_SCHEMA_VERSION = 1;

export const PUSH_NOTIFICATION_ANCHOR_DELIVERY_STATES = Object.freeze(['anchor-recorded'] as const);

export type PushNotificationAnchorDeliveryState =
  (typeof PUSH_NOTIFICATION_ANCHOR_DELIVERY_STATES)[number];

export const PUSH_NOTIFICATION_CHANNEL = 'push' as const;

export type DurablePushNotificationAnchor = Readonly<{
  workspaceId: string;
...
export type PushNotificationAnchorPersistenceOutcome =
  | Readonly<{ ok: true; anchor: DurablePushNotificationAnchor }>
...
/**
 * Build durable Push notification anchor for persistence (W5-N04-b).
 * Stores canonical notification anchors only — not Web Push/FCM transport or outbound delivery.
 */
export function buildPushNotificationAnchorState(input: {
  workspaceId: string;
  notificationId: string;
  notificationChannel: string;
  notificationType: string;
  ...
  actorId?: string | null;
  recordedAt: string;
  prior: DurablePushNotificationAnchor | null;
}): PushNotificationAnchorPersistenceOutcome {
  ...
  if (notificationChannel !== PUSH_NOTIFICATION_CHANNEL) {
    return Object.freeze({ ok: false, reason: 'notification_channel_must_be_push' });
  }
  ...
  const anchor = Object.freeze({
    ...
    schemaVersion: PUSH_NOTIFICATION_ANCHOR_SCHEMA_VERSION,
